...
def _import_legacy_runtime_data():
    ...
    Older checkouts stored runtime data below ``teamEvolver/skillminer`` or
    ``skillgene/skillminer``. Copy only files missing from ``team_miner``, so
    startup is idempotent and never overwrites uploads or generated artifacts.
    """
    runtime_dirs = (
    ...
    legacy_roots = (
        repository_root / "teamEvolver" / "skillminer",
        repository_root / "skillgene" / "skillminer",
    )
...
def _compiled_skill_details():
    ...
    for skill_dir in sorted(base.iterdir()):
        if not skill_dir.is_dir() or not (skill_dir / "SKILL.md").is_file():
            continue
        benchmark = skill_dir / "benchmark.json"
        legacy_benchmark = skill_dir / "benchmark.jsonl"
        question_count = 0
        if benchmark.is_file():
            payload, errors = rb.bf.read_document(benchmark)
            if payload is not None and not errors:
                question_count = len(payload.get("cases") or [])
        elif legacy_benchmark.is_file():
            question_count = sum(
                1 for line in legacy_benchmark.read_text(
        ...
        rows.append({
            ...
            "has_benchmark": benchmark.is_file() or legacy_benchmark.is_file(),
...
def _skill_summaries(artifacts):
    ...
    for artifact in artifacts:
        ...
        if name == "benchmark.json":
            ...
            payload, errors = rb.bf.read_document(target)
            if payload is not None and not errors:
                row["question_count"] = len(payload.get("cases") or [])
        elif name == "benchmark.jsonl":
            target = (PROJECT_ROOT / artifact["path"]).resolve()
...
class RunManager:
    ...
    def _run_benchmark(self, opts):
        ...
        try:
            ...
            if not questions:
                self.emit("bench_error", msg="题库为空（构建失败或未找到 benchmark.json）")
                self.state = "error"
                return
