...
from team_miner.settings import settings_payload, update_settings

SKILLMINER_DIR = Path(__file__).resolve().parents[1]
...
def test_progressive_benchmark_matches_skillgene_schema():
    ...
    assert payload["schema_version"] == "team-replay.dataset.v2"
    assert dataset["metadata"]["minimum_requirement_target"] == 12
    assert dataset["replay"]["progressive_disclosure"] == {
        "enabled": True,
...
def test_progressive_benchmark_repairs_short_rubrics_and_reports_review_warning():
    ...
    assert bf.validate_document(payload, expected_skill_name="demo") == []
    assert len([
        item for item in payload["cases"][0]["checks"]
        if item["kind"] == "output"
    ]) == 12
...
def test_hermes_environment_prefers_project_config_over_global_overrides(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TEAMEVOLVER_WORKSPACE_TOKEN", "tevt_tenantcredential")
    ...
    # A tenant machine credential is full-privilege for the tenant: the isolated
    # mining runtime must never inherit it.
    assert "TEAMEVOLVER_WORKSPACE_TOKEN" not in env
...
def test_pipeline_finalizer_builds_and_archives_benchmark_once(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        rp,
        "archive_round",
        lambda round_idx, session_tag: calls.append(("archive", round_idx, session_tag)) or (tmp_path / "round_1"),
    )
...
def test_mining_whitebox_settings_roundtrip_keeps_global_model():
    data = {
        "llm": {
            "provider": "custom",
    ...
    updated = update_settings(
        data,
        {
            ...
            "prompts": [{"id": "semantic_discovery", "prompt": "CUSTOM SEMANTIC"}],
        },
    )

    assert updated["model"]["model"] == "global-model"
    assert updated["model"]["base_url"] == "https://example.invalid/v1"
    assert "model" not in data["mining"]
    assert updated["pipeline"]["max_rounds"] == 7
    assert updated["pipeline"]["strict_step1"] is False
    semantic = next(item for item in updated["prompts"] if item["id"] == "semantic_discovery")
...
def test_benchmark_builder_limits_hermes_to_file_tools(tmp_path, monkeypatch):
    ...
    assert payload["cases"][0]["checks"][0]["id"] == "R01"
