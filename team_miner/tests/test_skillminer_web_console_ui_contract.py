...
SKILLMINER_ROOT = Path(__file__).resolve().parents[1]
...
from team_miner.settings import (  # noqa: E402
...
def test_mining_ui_has_no_separate_model_configuration_entry():
    ...
    mining_source = (project_root / "team_miner" / "frontend" / "MiningView.tsx").read_text(encoding="utf-8")
...
def test_mining_history_merges_rounds_with_next_run_preexisting_snapshot(tmp_path, monkeypatch):
    round_skill = (
        tmp_path / "reflection_rounds" / "20260805_150618_777997" / "round_1" / "compiled_skill" / "support-skill"
    )
    ...
    final_skill = (
        tmp_path / "run_history" / "20260809_223901_916214" / "preexisting" / "compiled_skill" / "support-skill"
    )
    ...
    report = tmp_path / "run_history" / "20260809_223901_916214" / "preexisting" / "semantic_reports" / "report.md"
...
def test_history_artifact_reader_is_path_safe(tmp_path, monkeypatch):
    ...
    result = server.read_history_artifact("reflection_rounds/run-1/round_1/compiled_skill/x/SKILL.md")
...
def test_upload_knowledge_converts_docx_to_markdown(tmp_path, monkeypatch):
    ...
    assert (tmp_path / ".knowledge_originals" / "input" / "售后手册.docx").read_bytes() == payload.getvalue()
...
def test_upload_knowledge_converts_xlsx_sheets_to_markdown(tmp_path, monkeypatch):
    ...
    assert (tmp_path / ".knowledge_originals" / "input" / "规则表.xlsx").read_bytes() == payload.getvalue()
...
def test_knowledge_sources_can_be_created_merged_and_deleted(tmp_path, monkeypatch):
    ...
    assert (tmp_path / ".knowledge_originals" / "combined" / "source-a" / "guide.docx").read_bytes() == b"docx-a"
    assert (tmp_path / ".knowledge_originals" / "combined" / "source-b" / "guide.docx").read_bytes() == b"docx-b"
...
def test_knowledge_source_can_be_renamed_without_overwriting_duplicate(tmp_path, monkeypatch):
    ...
    assert (tmp_path / ".knowledge_originals" / "customer-support" / "guide.docx").read_bytes() == b"original"
...
def test_job_artifact_reader_does_not_expose_snapshot_or_metadata(tmp_path, monkeypatch):
    ...
    assert (
        server.read_history_artifact("mining_jobs/job-1/workspace/compiled_skill/demo/SKILL.md")["content"] == "# Skill"
    )
    for forbidden in (
...
def test_skillminer_ui_omits_redundant_microcopy():
    ...
    mining_view = (repository_root / "team_miner" / "frontend" / "MiningView.tsx").read_text(encoding="utf-8")
    legacy_console = (repository_root / "team_miner" / "web_console" / "static" / "index.html").read_text(
        encoding="utf-8"
    )
...
def test_mining_task_deletion_contract_is_available_in_api_and_ui():
    repository_root = REPOSITORY_ROOT
    mining_view = (repository_root / "team_miner" / "frontend" / "MiningView.tsx").read_text(encoding="utf-8")
    server_source = (repository_root / "team_miner" / "web_console" / "server.py").read_text(encoding="utf-8")
...
def test_deleting_legacy_mining_job_removes_its_archived_outputs(tmp_path, monkeypatch):
    ...
    round_artifact = tmp_path / "reflection_rounds" / session_id / "round_1" / "compiled_skill" / "demo" / "SKILL.md"
    round_artifact.parent.mkdir(parents=True)
    round_artifact.write_text("# Demo", encoding="utf-8")
    final_artifact = (
        tmp_path / "run_history" / next_session_id / "preexisting" / "compiled_skill" / "demo" / "BENCHMARK.md"
    )
...
def test_new_source_upload_snapshots_files_and_uses_one_transaction():
    repository_root = REPOSITORY_ROOT
    mining_view = (repository_root / "team_miner" / "frontend" / "MiningView.tsx").read_text(encoding="utf-8")
    upload_handler = mining_view.split("async function uploadKnowledgeFiles", 1)[1].split("async function startRun", 1)[
        0
    ]
...
def test_mining_overview_exposes_source_totals_and_parallel_job_pipelines():
    repository_root = REPOSITORY_ROOT
    mining_view = (repository_root / "team_miner" / "frontend" / "MiningView.tsx").read_text(encoding="utf-8")
    overview = mining_view.split("{/* ---- 总览 ---- */}", 1)[1].split("{/* ---- 知识源 ---- */}", 1)[0]
...
def test_skillminer_user_facing_copy_uses_knowledge_source_terminology():
    ...
    for relative_path in (
        "team_miner/frontend/MiningView.tsx",
        "team_miner/web_console/server.py",
        "team_miner/run_coverage_report.py",
    ):
