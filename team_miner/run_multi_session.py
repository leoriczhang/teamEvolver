...
场景：
  ...
  每次出的题目可能不一样。本脚本负责在这 N 次之上做“稳定性复跑”：
    ...
    1) snapshot ：每次构建完成后，把当前 benchmark.json 快照留存为一个 build 版本；
...
子命令：
  snapshot   把当前 benchmark.json 存为一个 build 快照（--slot 指定序号，默认自增）
...
产物目录：benchmark_sessions/
  snapshots/build-1.json ...         每次构建的题库快照
...
def _load_snapshot_questions(path):
    """读取 canonical JSON 快照，并兼容历史 JSONL 快照。"""
    if not path.exists():
        return None
    if path.suffix == ".json":
        payload, errors = rb.bf.read_document(path)
        if payload is None or errors:
            return None
        return rb.normalize_questions(rb.bf.to_runner_questions(payload))
    rows = []
...
def _snapshot_slots():
    ...
    if not SNAP_DIR.exists():
        return []
    slots = []
    for p in [*SNAP_DIR.glob("build-*.json"), *SNAP_DIR.glob("build-*.jsonl")]:
        try:
...
def _snapshot_path(slot):
    canonical = SNAP_DIR / f"build-{slot}.json"
    return canonical if canonical.exists() else SNAP_DIR / f"build-{slot}.jsonl"


def cmd_snapshot(args):
    """把当前 canonical Test Dataset 存为一个 build 快照。"""
    skill_dir = _find_skill_dir()
    src = Path(args.from_file) if args.from_file else (skill_dir / "benchmark.json")
    if not src.exists() and not args.from_file:
        src = skill_dir / "benchmark.jsonl"
    if not src.exists():
        print(f"✗ 找不到题库文件：{src}")
        print("  提示：请先跑 `python3 run_benchmark.py --build-only` 生成 benchmark.json")
        sys.exit(1)

    questions = _load_snapshot_questions(src)
    if not questions:
    ...
    dst = SNAP_DIR / f"build-{slot}.json"

    payload, _warnings = rb.bf.build_document_with_quality(skill_dir.name, questions)
    rb.bf.write_document(dst, payload)
...
def _load_all_snapshots():
    ...
    for slot in _snapshot_slots():
        qs = _load_snapshot_questions(_snapshot_path(slot))
        if qs:
...
def parse_args():
    ...
    sp = sub.add_parser("snapshot", help="把当前 benchmark.json 存为一个 build 快照")
    sp.add_argument("--slot", type=int, default=None, help="快照序号（默认自增）")
    sp.add_argument("--from-file", dest="from_file", default=None,
                    help="指定题库来源文件（默认 compiled_skill/<skill>/benchmark.json）")
