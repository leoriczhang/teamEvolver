...
├── frontend/                           # 统一 React 控制台中的 Skill 挖掘视图
├── tests/                              # 模块回归与接口契约测试
├── web_console/                        # 标准库 SSE 控制台
├── api.py                              # 轨迹 Benchmark 的应用内公开接口
├── bridge.py                           # 主服务路由、子进程和反向代理适配
├── lifecycle.py                        # 挖掘产物进入候选评审的生命周期
├── settings.py                         # 白盒参数和 Prompt 配置
...
```bash
# 首次执行会初始化 team_miner/.hermes_home/config.yaml
scripts/project_hermes.sh --version
...
若希望修改“全新 checkout / 删除 `.hermes_home` 后”的默认值，可编辑可提交的 `team_miner/hermes/config.yaml.example`。API Key 请放在环境变量或 `.hermes_home/.env`，不要写入 YAML。
...
teamEvolver 内部组件应直接调用 Python API，无需通过 HTTP 回调自身：
...
```text
benchmark.json        # team-replay.dataset.v2 题库
...
产物位于 `benchmark_sessions/`：`snapshots/build-N.json` 快照、`intersection.md` 交集清单、`DIM_*/session-N.md` 每个 session 的完整对话与阅卷、`SESSIONS_REPORT.md` 逐维稳定性汇总。历史 `build-N.jsonl` 仍可读取，但不会再生成。
