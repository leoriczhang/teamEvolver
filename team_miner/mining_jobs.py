...
class MiningJobManager:
        ...
        if job.get("human_checkpoints", True):
            command.extend(
                [
                    "--human-checkpoints",
                    "--checkpoint-dir",
                    str(self._job_dir(job["job_id"]) / "checkpoints"),
                ]
            )
        env = dict(os.environ)
    ...
    def _assess_artifact_quality(self, job: dict[str, Any], *, return_code: int) -> dict[str, Any]:
        ...
        if evaluation.is_file() and evaluation.stat().st_size:
            artifacts.append(
                {"kind": "evaluation", "state": "ready", "label": "评测定义已生成", "detail": "可人工编辑与复核。"}
            )
        else:
            ...
            artifacts.append(
                {
                    ...
                    "state": "incomplete",
                    "label": "评测定义缺失",
                    "detail": "请补充 EVALUATION.md 后再提交。",
                }
            )

        if (
            benchmark_md.is_file()
            and benchmark_md.stat().st_size
            and benchmark_json.is_file()
            and benchmark_json.stat().st_size
        ):
            artifacts.append(
                {
                    "kind": "benchmark",
            ...
            artifacts.append(
                {
                    ...
                    "state": "incomplete",
                    "label": "Benchmark 不完整",
                    "detail": "可查看现有 Skill，但建议补齐题库后再提交。",
                }
            )
    ...
    def _knowledge_supplements(self, job: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if (
            reports_root.is_dir()
            and current_round not in archived_rounds
            and (
                current_round,
                "after_semantic",
            )
