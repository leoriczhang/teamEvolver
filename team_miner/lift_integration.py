...
try:
    from . import benchmark_format as bf
...
def list_source_skills() -> list[dict[str, Any]]:
    ...
    for skill_dir in sorted(base.iterdir()):
        ...
        if benchmark.is_file():
            payload, errors = bf.read_document(benchmark)
            if payload is not None and not errors:
                question_count = len(payload.get("cases") or [])
        elif legacy_benchmark.is_file():
            lines = legacy_benchmark.read_text(encoding="utf-8", errors="ignore").splitlines()
...
def load_benchmark_questions(skill_dir: Path) -> list[dict[str, Any]]:
    canonical_path = skill_dir / "benchmark.json"
    if canonical_path.is_file():
        payload, errors = bf.read_document(canonical_path)
        if payload is None or errors:
            raise ValueError("benchmark.json 格式无效：" + "；".join(errors))
        questions = bf.to_runner_questions(payload)
        if len(questions) < 2:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"缺少 benchmark.json 或 benchmark.jsonl: {skill_dir.name}")
    questions = []
