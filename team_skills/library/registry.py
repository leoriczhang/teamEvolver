...
class SkillIDRegistry:
    ...
    def save_to_oss(self, bucket, prefix: str) -> None:
        ...
        if bool(getattr(bucket, "native_batch_write", False)):
            _, precondition = self.merge_from_oss_snapshot(bucket, prefix)
            bucket.batch_write(
                {key: self.to_bytes()},
    ...
    def merge_from_oss_snapshot(
        ...
        changed = 0
        for name, incoming in self._normalise(value).items():
            current = self._map.get(name)
            if current is None or int(incoming.get("version") or 0) > int(
                current.get("version") or 0
            ):
                self._map[name] = incoming
                changed += 1
        if changed:
            self._dirty = True
        return changed, {
