...
class SkillIDRegistry:
    ...
    def merge_from_oss(self, bucket, prefix: str) -> int:
        ...
        The registry is loaded once at startup but written back at the end of
        every cycle, so a seed written to storage by an *external* producer
        (e.g. StaffDeck manually pushing local skills as team_skills.evolution's initial
        library) would otherwise be clobbered by the next save. Calling this
        before each save/status merges any storage entry that is new to us, or
        whose version is strictly higher than what we hold — never lowering a
        version this process has already advanced in memory. Returns the number
        of entries added or bumped.
        """
        key = f"{prefix}evolve_skill_registry.json"
        try:
            data = bucket.get_object(key).read().decode("utf-8")
    ...
    def _merge_raw(self, raw: dict[str, Any]) -> int:
        ...
        for name, entry in incoming.items():
            ...
            if current is None:
                ...
                continue
            try:
                if int(entry.get("version") or 0) > int(current.get("version") or 0):
                    self._map[name] = entry
                    changed += 1
            except (TypeError, ValueError):
                continue
        if changed:
            logger.info("[SkillIDRegistry] merged %d seeded entries from storage", changed)
        return changed
    ...
    def record_rollback(
        ...
        *,
        ...
        """Record a rollback as a new monotonic version pointing at old content.

        Version numbers stay monotonically increasing (so ``_detect_conflict``
        and downstream consumers keep working); the appended history entry is
        tagged ``action="rollback"`` and records which version was restored.
        """
        version = self.record_update(
            skill_name,
            content_sha,
            action="rollback",
            bundle_record=bundle_record,
        )
        history = self._map[skill_name].setdefault("history", [])
        if history:
            history[-1]["rolled_back_to"] = int(rolled_back_to)
        return version
