"""
Core orchestrator for the current session-level team_skills.evolution pipeline.
...
from team_replay.datasets.synthesis import (
    DATASET_FORMAT,
...
from team_replay.metrics import UNAVAILABLE, metric_number
from teamEvolver.observability import (
...
from team_skills.candidates import ValidationStore
...
from team_skills.evolution.kernel.bundle_changes import (
    BundleChangeError,
    materialize_bundle_changes,
    select_editable_files,
)
from team_skills.evolution.kernel.enums import NO_SKILL_KEY, DecisionAction
from team_skills.evolution.kernel.helpers import build_skill_md, parse_skill_content
...
from team_skills.evolution.stages.aggregate import aggregate_sessions_by_skill
from team_skills.evolution.stages.execute import (
...
from team_skills.evolution.store.object_store import (
    delete_session_keys,
    fetch_skill_bundle,
    fetch_skill_content,
...
from team_skills.evolution.runtime.evidence import SkillEvidenceStore, is_candidate_audit_session
...
class EvolveServer(EvolveEngineMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # Within this tenant, the Session queue is pooled across peers so team
        # evolution sees all of that tenant's evidence. Tenant isolation comes
        # from the bucket's PG tenant / storage prefix, not this peer-level path.
    ...
    @staticmethod
    def _skill_prefix_for_config(config: EvolveServerConfig) -> str:
        # Match SkillHub's tenant scope; PostgreSQL isolates through RLS.
        if config.skill_storage_backend != "postgres" and config.pg_tenant_id != "default":
            return f"tenants/{config.pg_tenant_id}/"
        return ""
    ...
    def _build_skill_bucket(
        ...
        *,
        ...
        backend_normalized = str(config.skill_storage_backend or "").strip().lower()
        ...
        return build_object_store(
            ...
            local_root=config.skill_storage_local_root or config.storage_local_root,
            allow_fallback=config.storage_fallback_enabled,
            fallback_root=config.skill_storage_local_root or config.storage_local_root,
        )
    ...
    async def _prepare_sessions(self, sessions: list[dict]) -> int:
        """Reuse complete ingest analysis and backfill legacy Sessions before evolution."""
        judged = 0
        if not sessions:
            return judged

        from team_skills.evolution.stages.analyze import analyze_session, session_has_merged_outputs

        judge_enabled = bool(self.config.use_session_judge)
        for session in sessions:
            if not session_has_merged_outputs(session):
                await analyze_session(self._llm, session)
            _extract_session_metadata(session)
            session["_trajectory"] = build_session_trajectory(session)
            # A session already analysed at ingest carries its judge overall in
            # ``_judge_scores``. ``_extract_session_metadata`` just recomputed
            # ``_avg_prm``/``_prm_scores`` from turn PRMs, so re-assert the
            # judge's overall the same way ``_apply_judge_scores`` does — keeping
            # the reused path's end-state identical to the standalone flow where
            # judge ran last.
            if session_has_merged_outputs(session):
                overall = session["_judge_scores"].get("overall_score")
                if isinstance(overall, (int, float)) and not isinstance(overall, bool):
                    session["_prm_scores"] = [overall]
                    session["_avg_prm"] = overall
                if judge_enabled:
                    judged += 1

        if judged:
            logger.info("[EvolveServer] reused %d merged session score(s)", judged)
        return judged
    ...
    def _queue_validation_job(
        ...
        *,
        ...
        runtime_preparation = prepare_runtime_validation(
            ...
            sessions=sessions,
            replay_cases=replay_cases,
            validation_runtimes=list(self.config.validation_runtimes),
        )
    ...
    def _validation_candidate_payload(
        ...
        *,
        ...
        def metric_int(value: Any, fallback: Any = UNAVAILABLE) -> Any:
            """Coerce one replay metric to ``int``, keeping the sentinel.

            DEAP cannot observe every metric, so True Replay records those as
            ``UNAVAILABLE`` (see :mod:`team_replay.metrics`); ``int()`` on that
            sentinel used to raise and 500 the whole candidate list.
            """
            if isinstance(value, str) and value.strip() not in {UNAVAILABLE, ""}:
                try:
        ...
        for name in metric_names:
            ...
            if baseline_value == UNAVAILABLE or candidate_value == UNAVAILABLE:
                dimensions[name] = {
        ...
        for raw_case in replay.get("cases") or []:
            ...
            for branch_name in ("baseline", "candidate"):
                ...
                normalized_case[branch_name] = {
                    ...
                    "interaction_turns": metric_int(
                        branch.get("interaction_turns"), fallback=None,
                    ),
                    "tool_call_count": metric_int(
                        branch.get("tool_call_count"), fallback=None,
                    ),
                    "total_tokens": metric_int(
                        branch.get("total_tokens"), fallback=None,
                    ),
    ...
    def _aggregate_replay_windows(
        ...
        *,
        ...
        result = aggregate_true_replay_windows(window_results)
        result["max_interactions"] = max_interactions
        result["reason"] = result.get("verdict", "inconclusive")
        return result
    ...
    def _read_session_ledger(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        from team_skills.evolution.store.object_store import list_object_keys
        ...
        for key in list_object_keys(self._bucket, prefix):
            ...
            if isinstance(entry, dict) and entry.get("session_id"):
                rows.append(entry)
        rows.sort(
            key=lambda r: (str(r.get("timestamp") or ""), str(r.get("ingested_at") or "")),
            reverse=True,
        )
        ...
        for session in sessions:
            ...
            archived = {
                **{key: value for key, value in session.items() if not key.startswith("_")},
                "_summary": session.get("_summary") or "",
                "_judge_scores": dict(session.get("_judge_scores") or {}),
    ...
    async def _run_observed_cycle(self) -> dict:
        ...
        with langfuse_observation(
            name="team_skills.evolution.cycle",
            as_type="agent",
            input={
                "cycle_id": cycle_id,
            ...
            trace_name="team_skills.evolution.cycle",
            session_id=cycle_id,
            tags=["evolve", "cycle"],
        ) as observation:
    ...
    def create_http_app(self):
        ...
        async def skill_version_detail(name: str, version: int):
            ...
            if status_code == 200:
                try:
                    from teamEvolver.proxy.skills_admin import _version_evolution_context
