...
from team_skills.library.bundle import candidate_skill_bundle, encode_bundle_payload
from teamEvolver.storage import build_object_store, is_not_found_error, peer_key_prefix
...
class ValidationStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._base_prefix = str(prefix or "").strip("/")
    ...
    def from_bucket(
        ...
        *,
        ...
        store._base_prefix = str(prefix or "").strip("/")
        return store
    ...
    def from_config(cls, config, tenant_id: str = "default") -> "ValidationStore":
        ...
        store._base_prefix = hub._prefix().strip("/")
        return store

    def _prefix(self) -> str:
        parts = []
        if self._base_prefix:
            parts.append(f"{self._base_prefix}/")
    ...
    def _replay_has_completed_metrics(source: dict[str, Any]) -> bool:
        ...
        if not isinstance(dimensions, dict) or not dimensions:
            return False
        # DEAP cannot observe every metric, so True Replay records those as the
        # ``UNAVAILABLE`` sentinel string; ``int()`` on it raises and would 500
        # the candidate list (and blank the review queue) for an otherwise
        # perfectly good evaluation.
        from team_replay.metrics import metric_number

        return any(
            isinstance(metric, dict)
            and (
                bool(metric_number(metric.get("baseline")))
                or bool(metric_number(metric.get("candidate")))
            )
