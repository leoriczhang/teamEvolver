...
async def run_ingest(cfg, sessions_dir: Path, run_dir: Path, concurrency: int) -> dict:
    ...
    Judging is deliberately decoupled from the evolution trigger: every session
    with meaningful content goes through the merged ``analyze_session`` call
    (classification + summary + judge in one shot) so downstream consumers get
    scores for the full population, while only ``valuable`` sessions enter the
    evolution queue. Queued sessions carry ``_judge_scores`` so the evolve
    cycle reuses them (``_prepare_sessions`` is a cache hit).
    """
    from team_skills.evolution.session_filter import SessionValueClassifier
    from team_skills.evolution.stages.analyze import analyze_session
    from team_skills.evolution.stages.judge import judge_has_defect_evidence
    ...
    # Trace-level results: one JSONL row per trace, appended as sessions are
    # analyzed. The evolution hook fires per result too; this file is the
    # offline record from which the BadCase-only CSV export is rendered.
    trace_badcase_path = run_dir / "trace_badcases.jsonl"
    trace_badcase_fh = trace_badcase_path.open("a", encoding="utf-8")
    ...
    async def analyze(session: dict) -> dict:
        """Merged classify + summary + judge; scores land on the session.

        Returns the ``value_judge`` verdict from a complete model analysis.
        After scoring, every session gets per-trace analysis; each result fires
        the registered hook and is buffered on the session under
        ``_trace_badcases`` for the offline JSONL/CSV record.
        """
        async with sem:
            verdict = await analyze_session(classifier.client, session)
            # Mandatory trace analysis dispatches the per-result hook and
            # returns one model result per trace. Any failure stops the run.
            session["_trace_badcases"] = await analyze_and_dispatch_traces(
                classifier.client, session
            )
            return verdict
    ...
    for offset in range(0, len(files), chunk):
        ...
        # One merged call per non-empty session: classify + summary + judge.
        judges = await asyncio.gather(
            *(
                analyze(p) if p and _has_meaningful_content(p) else _nowait(None)
                for p in payloads
            )
        )
        ...
        for f, payload, judge in zip(batch_files, payloads, judges):
            ...
            if judge is None:
                ...
                continue
            # Record every trace-level result produced for this session.
            for trace_row in payload.pop("_trace_badcases", None) or []:
                if not isinstance(trace_row, dict):
                    continue
                trace_badcase_fh.write(
                    json.dumps(trace_row, ensure_ascii=False) + "\n"
                )
                if trace_row.get("is_badcase"):
                    counts["trace_badcases"] += 1
            try:
                ...
                # Preserve complete model outputs; derived metadata includes
                # sets and is recomputed by the evolution cycle.
                for key in list(payload):
                    if key.startswith("_") and key not in {"_judge_scores", "_summary"}:
                        del payload[key]
...
def export_trace_badcases_csv(run_dir: Path) -> Path | None:
    """Render the trace-level BadCase JSONL into a flat CSV.

    Reads ``trace_badcases.jsonl`` (one row per analyzed trace, written during
    ingest) and emits ``trace_badcases.csv`` with the Trace-level 问题类型 /
    问题描述 and both timestamps (业务发生时间 vs 系统入库时间). Returns the CSV
    path, or ``None`` when no trace analysis ran.
    """
    jsonl_path = run_dir / "trace_badcases.jsonl"
    if not jsonl_path.exists():
        return None
    columns = [
        "session_id", "trace_id", "turn_num", "is_badcase",
        "problem_type", "problem_type_label", "problem_description",
        "business_time", "ingested_at",
        "user_id", "session_trace_id",
        "session_overall_score", "source", "analyzed_at",
    ]
    trace_csv = run_dir / "trace_badcases.csv"
    n_rows = 0
    with open(trace_csv, "w", newline="", encoding="utf-8-sig") as fh:
        ...
        for line in jsonl_path.read_text(encoding="utf-8").splitlines():
            ...
            if not isinstance(rec, dict):
                continue
            # Only surface traces flagged as problems; the JSONL also carries
            # non-badcase traces (one row per trace) for the hook contract.
            if not rec.get("is_badcase"):
                continue
            writer.writerow(rec)
            n_rows += 1
    print(f"[export] trace badcases: {n_rows} rows", flush=True)
    return trace_csv
...
def main() -> int:
    ...
    if args.export_only:
        ...
        trace_csv = export_trace_badcases_csv(run_dir)
    ...
    if args.requeue_defective:
        ...
        export_trace_badcases_csv(run_dir)
        return 0
