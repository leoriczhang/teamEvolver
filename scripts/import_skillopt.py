...
from team_replay.adapter_runtime import AdapterError, read_content, save_content
from team_skills.library.bundle import bundle_tree_sha256, read_skill_bundle
...
def converter_preflight(directory: Path, project: str, raw: dict, *, allow_doris_api_fallback=False):
    ...
    if (raw.get("langfuse") or {}).get("project_id") and not allow_doris_api_fallback:
        result["issues"].append(
            "Doris source requires explicit --allow-doris-api-fallback after "
            "API parity verification, or a custom SourceAdapter"
        )
    ...
    for path, raw, code, converter in prepared:
        ...
        if apply:
            ...
            try:
                ...
                if endpoint:
                    filename = "skillopt-" + hashlib.sha256(account.encode()).hexdigest()[:16] + ".py"
                    content = (
                        "from team_replay.factories import DeapReplayFactory\n"
                        'REPLAY_ADAPTER = {"label": "Skillopt DEAP", "enabled": True}\n'
                        "def build_replay_adapter(config):\n"
                        "    return DeapReplayFactory("
                        f"endpoint={endpoint!r}, "
                        f"employee_no={str(experiment.get('emp_id') or '')!r})\n"
                    )
                    try:
                        current = read_content(config, filename)
                    except AdapterError:
                        current = None
                    if current and current["code"] != content:
                        raise ValueError("Existing Replay adapter differs; review it before importing again")
                    if current is None:
                        save_content(config, filename, content)
                    registry.update_tenant_config(account, {"replay_adapter": filename})
            finally:
