...
- ``single``: storage_pg is not enabled — only the implicit ``default``
  ...
  keep their exact single-tenant behavior (zero-change compatibility).
...
from ..storage.pg_pool import _DEFAULT_COMMAND_TIMEOUT, _DEFAULT_POOL_MAX, _DEFAULT_POOL_MIN, dsn_from_env, get_pg_runtime
...
def effective_config(registry: "TenantRegistry", ctx: TenantContext | None, base: Any) -> Any:
    ...
    if bool(getattr(base, "storage_pg_enabled", False)):
        tid = ctx.tenant_id if ctx is not None else DEFAULT_TENANT_ID
...
class TenantRegistry:
    ...
    def __init__(
        ...
        *,
        ...
        if self._runtime is None and config is not None and bool(
            ...
            if not dsn:
                raise ValueError("storage_pg.enabled requires a DSN or OV_PG_HOST")
            if dsn:
                self._runtime = get_pg_runtime(
                    dsn=dsn,
                    schema=str(getattr(config, "storage_pg_schema", "") or "teamevolver"),
                    pool_min=max(1, int(getattr(config, "storage_pg_pool_min", _DEFAULT_POOL_MIN) or 2)),
                    pool_max=max(2, int(getattr(config, "storage_pg_pool_max", _DEFAULT_POOL_MAX) or 20)),
    ...
    def resolve_by_agent_token(self, token: str) -> TenantContext | None:
        """Map a ``tevt_`` agent token to its tenant (cached, incl. negatives)."""
        token = str(token or "").strip()
        if not self._pg_enabled or not token.startswith(AGENT_TOKEN_PREFIX):
            return None
