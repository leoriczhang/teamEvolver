...
from .cards import build_merge_card, build_round1_user_msg, render_observation_message
...
from .tools import ToolContext, ToolRegistry, build_protocol_appendix
...
def _dump(stem: str, suffix: str, system: str, user_msg: str, raw: Optional[str]) -> None:
    if not stem:
        return
    try:
        from ..stages.execute import _write_debug_dump
...
def _prevalidate_file_changes(skill: dict, ctx: ToolContext) -> str:
    ...
    try:
        from ...skills.bundle import candidate_skill_bundle
        from ..kernel.bundle_changes import materialize_bundle_changes
