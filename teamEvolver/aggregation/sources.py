...
1. Enumerates real users under an OpenViking account via the Admin API
   (request-scoped Admin Key), mirroring the request shape already used by
...
class AccountSourceBuilder:
    ...
    async def list_account_users(self) -> list[str]:
        ...
        Uses the OpenViking Admin API with the request-scoped Admin Key.
        ...
        if not self.admin_key:
            raise SourceExpansionError(
                "aggregation requires an OpenViking Admin Key"
            )
