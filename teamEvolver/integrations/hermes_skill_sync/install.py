...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--min-interval-seconds", type=int, default=15, help="minimum pull interval before model calls")
    ...
    parser.add_argument("--user-id", "--user", dest="user_id", default="", help="tenant-scoped user ID")
    parser.add_argument(
        "--tenant-token", "--api-key", dest="tenant_token",
        default=os.environ.get("TEAMEVOLVER_TENANT_TOKEN", ""), help="tenant machine credential (tevt_...)",
    )
    parser.add_argument(
        "--viking-deployment",
        default="cloud",
    ...
    if args.backend == "service":
        if not (args.url or args.service_url).strip() or not args.user_id.strip():
            parser.error("--url and --user-id are required for service pull")
        if not args.tenant_token.startswith("tevt_"):
