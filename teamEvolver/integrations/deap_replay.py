...
from ..skills.bundle import decode_bundle_payload, normalize_bundle_rel_path
from .agent_protocol import REPLAY_TURN_RESULT_SCHEMA_V1
from .replay_adapters import resolve_replay_api_key
...
class DeapReplayAdapter:
    ...
    def _prepare(self, skill):
        ...
        texts = {
            normalize_bundle_rel_path(path): content.decode("utf-8") if isinstance(content, bytes) else str(content)
    ...
    def call_turn(self, request):
        ...
        try:
            ...
            result.update(status="succeeded", final_response=body["answer"])
