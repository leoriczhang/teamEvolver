...
from team_skills.library.runtime_policy import skill_supports_runtime
from .agent_protocol import CAP_SKILL_SYNC
from .agent_registry import list_agents

# Non-agent consumer ids that may appear in event["deliveries"] alongside
# per-agent sync records; they must not be treated as deregistered agents.
SKILLOPT_ROLLOUT_CONSUMER = "skillopt_rollout"
...
async def sync_skill_event(
    ...
    for agent_id in (
        set(event.get("deliveries") or {})
        - known_agents
        - {SKILLOPT_ROLLOUT_CONSUMER}
    ):
        results[agent_id] = {
            "status": "cancelled",
