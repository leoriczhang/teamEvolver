...
def client_config(config: dict[str, Any]) -> dict[str, Any]:
    """Canonical client settings; old setting names are read during rollout."""
    return {
        **config,
        "base_url": str(os.environ.get("TEAMEVOLVER_URL") or config.get("base_url") or "").strip(),
        "tenant_token": str(
            os.environ.get("TEAMEVOLVER_TENANT_TOKEN")
            or os.environ.get("TEAMEVOLVER_WORKSPACE_TOKEN")
            or config.get("tenant_token") or config.get("workspace_token") or ""
        ).strip(),
        "user_id": str(
            os.environ.get("TEAMEVOLVER_USER_ID") or os.environ.get("TEAMEVOLVER_USER")
            or config.get("user_id") or config.get("user_alias") or ""
        ).strip(),
    }


def producer_id(config: dict[str, Any]) -> str:
    """Partition local deliveries by destination and subject, never authorize them."""
    config = client_config(config)
    scope = [config["base_url"].rstrip("/"), config["tenant_token"], config["user_id"]]
    return "hermes:" + hashlib.sha256(json.dumps(scope).encode("utf-8")).hexdigest()[:24]
...
class HermesDeliverySpool:
    def __init__(self, path: Path, *, producer_id: str) -> None:
        self.path = path.expanduser()
        self.producer_id = str(producer_id or "hermes:local")
    ...
    def enqueue(
        ...
        *,
        ...
        with self._locked():
            ...
            now = time.time()
            delivery = {
                "schema_version": "teamevolver.hermes-delivery.v2",
    ...
    def _records(self) -> list[dict[str, Any]]:
        records = [
            ...
            and record.get("producer_id") == self.producer_id
        ]
        priority = {"context.start": 1}
        return sorted(
            records,
            key=lambda item: (
    ...
    def _blocked(self, delivery: dict[str, Any]) -> bool:
        for candidate in self._records():
            if candidate.get("delivery_id") == delivery.get("delivery_id"):
                continue
            if (
                candidate.get("producer_id") == delivery.get("producer_id")
    ...
    def deliver(
        ...
        *,
        ...
        with self._locked():
            ...
            if delivery.get("producer_id") != self.producer_id:
                raise ValueError("delivery belongs to another producer")
