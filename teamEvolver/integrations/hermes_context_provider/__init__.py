...
def _load_feed() -> dict[str, Any]:
    ...
    try:
        ...
        value = {}
    return client_config(value if isinstance(value, dict) else {})
...
class TeamEvolverMemoryProvider(MemoryProvider):
    ...
    def is_available(self) -> bool:
        config = _load_feed()
        token = str(config.get("tenant_token") or "").strip()
        return bool(
            str(config.get("base_url") or "").strip()
            # Only the tenant machine credential works; a retired per-Agent
            # token must not make the provider look usable.
            and token.startswith("tevt_")
            and str(config.get("user_id") or "").strip()
            and not str(os.environ.get("TEAMEVOLVER_DISABLE_SESSION_FEED") or "").strip()
        )
        ...
        *,
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "Authorization": (
                    "Bearer "
                    + str(self._config.get("tenant_token") or "")
                ),
            },
        )
        try:
            ...
            if isinstance(exc, urllib.error.HTTPError):
                try:
                    detail = f" body={exc.read().decode('utf-8', 'replace')[:200]}"
                except (OSError, ValueError):
                    detail = ""
            raise RuntimeError(
                f"teamEvolver Context request failed: {exc}{detail}"
            ) from exc
    ...
    def _identity(self) -> dict[str, str]:
        return {"user_id": str(self._config.get("user_id") or "")}
    ...
    def _delivery_spool(self) -> HermesDeliverySpool:
        ...
        return HermesDeliverySpool(
            ...
            producer_id=producer_id(self._config),
        )
        ...
        **kwargs,
        ...
        if tool_name == "team_evolver_context_read":
            ...
            result = self._request(
                ...
                body={"context_ref": str(args.get("context_ref") or ""), **self._identity()},
            )
        else:
