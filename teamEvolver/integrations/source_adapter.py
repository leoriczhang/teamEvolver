...
pattern proven in the skill-opt project:
  ...
  - **Data-source layer** (how to list sessions, fetch details, convert
    raw traces) is provided by a ``SourceAdapter`` implementation. The
    built-in default is ``LangfuseSourceAdapter``.
  - **Per-agent layer** (business-specific filtering, field extraction,
    dedup, custom conversion) is an optional file in ``adapters/<agent_id>.py``
...
# Defaults to the repo-bundled adapters/ directory so they ship with the code.
# Override with config ``datasource.adapters_dir`` for a custom location.
_REPO_ADAPTERS_DIR = Path(__file__).resolve().parents[2] / "adapters"
...
def build_source_adapter(config: Any) -> SourceAdapter:
    ...
    Defaults to ``"langfuse"``. Unknown types fall back to langfuse with
    a warning so a misconfiguration never hard-crashes the pipeline.
    """
    source_type = str(
    ...
    if factory is None:
        ...
        factory = _ADAPTER_CLASSES["langfuse"]
...
def adapter_path(agent_id: str, config: Any = None) -> Optional[Path]:
    ...
    Looks for ``adapters/<agent_id>.py``. Returns None when no file exists
...
def resolve_hooks(
    ...
    hooks = AgentHooks.from_module(module, source=str(path))
    with _hook_lock:
