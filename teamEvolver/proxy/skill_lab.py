...
from team_replay.datasets.schema import legacy_case_view
from team_replay.datasets.synthesis import synthesize_evolution_datasets
from team_replay.lab.service import (
    SkillLabError,
    SkillLabStore,
    evolution_datasets,
...
def _hub_skill_md(owner: Any, skill_name: str) -> str:
    """Read a skill's SKILL.md from the team library (no local working copy)."""
    from .skill_workspace import team_hub

    bundle = team_hub(owner).read_skill_bundle(skill_name)
    return bundle.get("SKILL.md", b"").decode("utf-8", errors="replace")


def _hub_load_bundle(owner: Any):
    """Bundle loader for experiment baselines, resolved from the team library."""
    from .skill_workspace import team_hub

    def load(name: str):
        return team_hub(owner).read_skill_bundle(name)
...
class SkillLabMixin:
        ...
        *,
        ...
        try:
            ...
            result = await asyncio.to_thread(
                EmbeddedReplayGateway(evaluate_job).evaluate,
                spec,
                case_index=0,
    ...
    def _register_skill_lab_routes(self, app: FastAPI) -> None:
        owner = self
        from .skill_workspace import register_skill_workspace_routes

        register_skill_workspace_routes(owner, app)
        ...
        @app.get("/api/replay-lab/datasets")
        ...
        @app.post("/api/replay-lab/datasets")
        ...
        @app.post("/api/replay-lab/datasets/synthesize")
        async def api_skill_lab_synthesize_datasets(body: dict[str, Any]):
            ...
            try:
                ...
                candidate_skill = parse_skill_markdown(
                    _hub_skill_md(owner, skill_name)
                )
                ...
                llm = AsyncLLMClient(
                    ...
                    max_concurrency=int(
                        getattr(_cfg, "llm_max_concurrency", 8) or 8
                    ),
                    queue_capacity=int(
                        getattr(_cfg, "llm_queue_capacity", 64) or 64
                    ),
                )
                ...
                store = lab_store()
                generated_views = [
                    (dataset, legacy_case_view(dataset))
                ...
                saved = [
                    store.save_dataset(
                        {
                            "skill_name": skill_name,
                            "dataset_id": dataset.get("case_id"),
                            "name": dataset.get("name"),
                            "query": dataset.get("query"),
                            "requirements": view.get("requirements"),
                            "trajectory_requirements": view.get(
                                "trajectory_requirements"
                            ),
                            "progressive_disclosure": view.get(
                                "progressive_disclosure"
                            ),
                            "source": {
                                "kind": "synthesized",
                                "source_session_ids": view.get(
                                    "source_session_ids"
                                )
                                or [],
                                "evidence_window": view.get(
                                    "evidence_window"
                                ),
                                "candidate_job_id": latest.get("job_id"),
                                "synthesis_mode": view.get(
                                    "synthesis_mode"
                                ),
                            },
                    ...
                    for dataset, view in generated_views
                ]
        ...
        @app.post("/api/replay-lab/datasets/{dataset_id}/clone")
        async def api_skill_lab_clone_dataset(
            ...
            try:
                ...
                cloned = store.save_dataset(
                    {
                        "skill_name": skill_name,
                        "skill_ids": source.get("skill_ids") or [skill_name],
        ...
        @app.delete("/api/replay-lab/datasets/{dataset_id}")
        ...
        @app.get("/api/replay-lab/runs/{run_id}")
        ...
        async def api_skill_lab_start_run(body: dict[str, Any]):
            ...
            try:
                ...
                job = prepare_experiment_job(
                    load_bundle=_hub_load_bundle(owner),
                    skill_name=skill_name,
                    candidate_skill_md=candidate_skill_md,
                    candidate_files=body.get("candidate_files"),
                    dataset=dataset,
                    materials=materials,
                    run_id=run_id,
                )
                record = store.create_run(
                    {
                        ...
                        "candidate_skill_md": candidate_skill_md,
                        "candidate_files": body.get("candidate_files") or {},
                        "timeout_seconds": timeout_seconds,
                        "max_interactions": max_interactions,
                        "status": "running",
                    }
        ...
        @app.post("/api/replay-lab/skills/{skill_name}/save")
    ...
    def _save_skill_lab_draft(self, skill_name: str, raw: str) -> dict[str, Any]:
        """Commit an experimented draft into the team skill library.

        Writes SKILL.md through the skill hub (OpenViking, or PostgreSQL when
        OpenViking is unavailable) so the workbench, the version modal and the
        dashboard all read the same version — there is no local working copy.
        """
        from .skill_workspace import (
            _enqueue_delivery,
        ...
        hub = team_hub(self)
        try:
            current = hub.read_skill_file(skill_name, "SKILL.md")
        except FileNotFoundError as exc:
            raise SkillLabError(f"Skill 不存在：{skill_name}") from exc
        entry = hub.write_skill_file(
            skill_name,
            "SKILL.md",
            raw.encode("utf-8"),
        ...
        cloud = _enqueue_delivery(self, skill_name, dict(entry))
