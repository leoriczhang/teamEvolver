...
One :class:`~team_skills.evolution.runtime.orchestrator.EvolveServer` instance per
...
class EnginePool:
    ...
    def build_evolve_config(self, tenant_id: str) -> Any:
        ...
        # Quota: max_concurrent_sessions caps this tenant's parallel session
        # groups inside its own LLM dispatcher.
    ...
    def _get_or_build(self, tenant_id: str) -> Any | None:
        ...
        with self._lock:
            ...
            if len(self._engines) >= self._max:
                ...
                value.stop()
        try:
            from team_skills.evolution import EvolveServer
