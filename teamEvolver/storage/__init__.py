...
def pg_dsn_available(dsn: str = "") -> bool:
    """True when a PostgreSQL DSN is configured (explicit or ``TEAMEVOLVER_PG_*``)."""
    try:
        return bool(str(dsn or "").strip() or dsn_from_env())
    except Exception:  # noqa: BLE001 - a bad env block must not break store setup
        return False
...
def build_object_store(
    *,
    ...
    fallback_root: str = "",
    # Fallback target when the viking backend is unavailable: ``local`` (built-in
    # filesystem store, the default) or ``postgres`` (the multi-tenant local
    # state store, used by the shared skill library so every console page reads
    # one source). ``postgres`` is honoured only when a DSN is configured.
    ...
    local-state store (dedicated schema on the project's pgm instance;
    ``pg_dsn`` empty → derived from ``TEAMEVOLVER_PG_*`` env vars).
    ...
    ``local_root`` doubles as an alias for ``fallback_root`` in that case.
    ``fallback_backend="postgres"`` swaps that fallback for the PostgreSQL
    object store when a DSN is configured (used by the shared skill library so
    every console page reads one source through an outage). For an in-process
    test/mock double, construct
    ...
    if resolved == "viking":
        ...
        if allow_fallback:
            ...
            if not available:
                if normalize_backend(fallback_backend) == "postgres" and pg_dsn_available(pg_dsn):
                    logger.warning(
                        "OpenViking %s unavailable (%s) — falling back to the "
                        "PostgreSQL object store (tenant=%s)",
                        endpoint,
                        reason,
                        tenant_id,
                    )
                    fallback_store = PgObjectStore(
                        dsn=pg_dsn,
                        schema=pg_schema,
                        tenant_id=tenant_id,
                        pool_min=pg_pool_min,
                    ...
                    # Stash provenance so status endpoints can explain the
                    # fallback without re-probing the dead endpoint.
                    fallback_store.fallback_active = True
                    fallback_store.fallback_reason = reason
                    fallback_store.fallback_endpoint = endpoint
                    fallback_store.fallback_backend = "postgres"
                ...
                fallback_store.fallback_reason = reason
                fallback_store.fallback_endpoint = endpoint
                fallback_store.fallback_backend = "local"
                return fallback_store
        return store
