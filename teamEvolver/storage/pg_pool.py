...
The DSN targets the project's dedicated PostgreSQL pgm instance
(``TEAMEVOLVER_PG_*`` environment variables, see ``.env.example``). Passwords
containing reserved characters (e.g. ``#``) are percent-encoded per the
OpenViking PostgreSQL guide §4.1.
"""
...
def dsn_from_env() -> str:
    """Derive the DSN from the ``TEAMEVOLVER_PG_*`` environment variables.

    The project runs on its own dedicated pgm instance (see ``.env`` /
    ``.env_sit`` / ``.env_prd``); the group is taken as a whole. Returns an
    empty string when ``TEAMEVOLVER_PG_HOST`` is unset, letting callers fall
    back to explicit configuration.
    """
    import os

    host = str(os.environ.get("TEAMEVOLVER_PG_HOST") or "").strip()
    if not host:
        return ""
    return build_pg_dsn(
        host=host,
        port=os.environ.get("TEAMEVOLVER_PG_PORT") or 5432,
        database=os.environ.get("TEAMEVOLVER_PG_DATABASE") or "teamevolver",
        username=os.environ.get("TEAMEVOLVER_PG_USERNAME") or "teamevolver",
        password=os.environ.get("TEAMEVOLVER_PG_PASSWORD") or "",
    )
...
_CONFIG_KV_TABLE_SQL = """
CREATE TABLE IF NOT EXISTS {schema}.config_kv (
...
async def init_pg_schema(conn: Any, schema: str = _DEFAULT_SCHEMA) -> None:
    ...
    exposing ``execute(sql)`` / ``fetchval(sql, ...)`` (asyncpg connection or
    test fake). The schema is probed via ``information_schema`` first: on the
    managed pgm instances the app role has no database-level CREATE and the
    schema is pre-created by a DBA, and on PostgreSQL 18 even
    ``CREATE SCHEMA IF NOT EXISTS`` consults the database ACL when the schema
    already exists. Callers hold the schema-init advisory transaction lock,
    so probe-then-create stays race-safe across replicas.
    """
    ...
    statements += [
        ...
        *(sql.format(schema=schema).replace(".objects", ".session_index") for sql in _RLS_SQL),
        ...
        _CONFIG_KV_TABLE_SQL.format(schema=schema),
    ]
...
class PgRuntime:
    ...
    # -- config_kv (model config persistence) ------------------------------- #

    async def _config_kv_get(self, key: str) -> Any:
        pool = await self._ensure_pool()
        async with pool.acquire(timeout=self._command_timeout) as conn:
            await conn.execute(
                "SELECT set_config('search_path', $1, false)", self._schema
            ...
            row = await conn.fetchrow(
                f"SELECT value FROM {self._schema}.config_kv WHERE key = $1", key
            )
            return row["value"] if row else None

    async def _config_kv_put(self, key: str, value: Any) -> None:
        import json
        pool = await self._ensure_pool()
        async with pool.acquire(timeout=self._command_timeout) as conn:
            await conn.execute(
            ...
            await conn.execute(
                f"INSERT INTO {self._schema}.config_kv (key, value) "
                "VALUES ($1, $2::jsonb) "
    ...
    def config_get(self, key: str, *, timeout: float = 10.0) -> Any:
        """Read a JSON value from ``config_kv``; returns ``None`` if missing."""
        try:
            return self.run(self._config_kv_get(key), timeout=timeout)
        except Exception:  # noqa: BLE001
            return None

    def config_put(self, key: str, value: Any, *, timeout: float = 10.0) -> bool:
        """Write a JSON value to ``config_kv``; returns ``False`` on failure."""
        try:
            self.run(self._config_kv_put(key, value), timeout=timeout)
            return True
        except Exception:  # noqa: BLE001
            return False
...
def get_pg_runtime(
    *,
    ...
    """Process-wide runtime registry keyed by (dsn, schema, pool sizes, ssl)."""
    schema = validate_schema_name(schema)
    key = (dsn, schema, int(pool_min), int(pool_max), float(command_timeout), ssl)
    with _RUNTIME_LOCK:
