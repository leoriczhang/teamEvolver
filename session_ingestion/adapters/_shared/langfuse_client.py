...
class LangfuseClient:
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            try:
                ...
                if attempt >= _MAX_ATTEMPTS:
                    raise LangfuseError(
                        f"Langfuse request to {path} failed: {exc}"
                    ) from exc
                delay = self._backoff_delay(attempt)
                self._record_retry(path, "network_error", attempt, delay)
                time.sleep(delay)
                continue
            ...
            if (
                ...
                self._record_retry(path, response.status_code, attempt, delay)
                time.sleep(delay)
                continue
            break

        if response is None:
            raise LangfuseError(f"Langfuse request to {path} failed")
    ...
    def _record_retry(
        ...
        self._retry_count += 1
        logger.warning(
            "[Langfuse] retrying GET path=%s status=%s attempt=%d/%d delay=%.3fs",
            path,
            status,
            attempt,
            _MAX_ATTEMPTS,
    ...
    def _backoff_delay(attempt: int) -> float:
        return min(
            ...
            0.5 * (2 ** max(0, attempt - 1)) + random.uniform(0, 0.25),
        )
    ...
    def _retry_delay(cls, response: httpx.Response, attempt: int) -> float:
        if response.status_code == 429:
            ...
            if value.isdigit():
                return min(_MAX_RETRY_DELAY_SECONDS, float(value))
            if value:
                try:
