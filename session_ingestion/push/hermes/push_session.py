...
Idempotent: teamEvolver keys v2 Sessions by tenant, user and external session ID,
so firing on every turn refreshes the latest snapshot for that subject.
"""
...
try:
    from teamEvolver.integrations.hermes_delivery import HermesDeliverySpool, client_config, producer_id
except ImportError:
    from hermes_delivery import HermesDeliverySpool, client_config, producer_id
...
def _read_session(db_path: Path, session_id: str) -> dict | None:
    # The installer imports this module too; SQLite is needed only on the host
    # actually reading Hermes transcripts, not when preparing the client bundle.
    try:
        import sqlite3
    except ImportError:
        _log("this Python interpreter has no SQLite support; use a Python build with sqlite3")
        return None
...
def _v2_session(session: dict, cfg: dict, user: str) -> dict:
...
def _delivery_sender(
    ...
    *,
    ...
    def send(delivery: dict) -> dict:
        ...
        tenant_token = str(cfg.get("tenant_token") or "")
        if not tenant_token.startswith("tevt_"):
            raise RuntimeError("tenant_token (tevt_) is required")
...
def main() -> int:
    ...
    cfg = client_config(_load_config())
    user = cfg["user_id"]
    if not user:
        _log("no user_id configured (run install.py or set TEAMEVOLVER_USER_ID)")
        return 0
    ...
    if not base_url:
        ...
        return 0
    if not cfg["tenant_token"].startswith("tevt_"):
        _log("tenant_token (tevt_) is required; run install.py --tenant-token ...")
        return 0
    ...
    session = _v2_session(session, cfg, user)
