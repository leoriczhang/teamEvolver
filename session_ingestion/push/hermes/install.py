...
Portable: run this on any machine that has Hermes. It
  ...
  2. writes ``feed.json`` with base_url, tenant_token and user_id,
...
Usage::
    ...
    python install.py --user-id alice --url http://evolve-host:52010 --tenant-token <tevt_token>
...
def _integration_assets_dir() -> Path:
    return Path(__file__).resolve().parents[3] / "teamEvolver" / "integrations"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--user-id", "--user", dest="user_id", required=True, help="tenant-scoped user ID")
    parser.add_argument(
        "--url",
        required=True,
    ...
    parser.add_argument(
        "--tenant-token", "--workspace-token",
        dest="tenant_token",
        default=os.environ.get("TEAMEVOLVER_TENANT_TOKEN", os.environ.get("TEAMEVOLVER_WORKSPACE_TOKEN", "")),
        help="tenant machine credential (tevt_...) for Session and Context",
    )
    parser.add_argument(
        "--no-context-provider",
        action="store_true",
    ...
    integration_assets = _integration_assets_dir()
    ...
    try:
        ...
        existing_feed = {}
    token = str(
        args.tenant_token or existing_feed.get("tenant_token") or existing_feed.get("workspace_token") or ""
    ).strip()
    if not token.startswith("tevt_"):
        parser.error("--tenant-token must be a tenant machine credential (tevt_)")
    feed = {"base_url": args.url.rstrip("/"), "tenant_token": token, "user_id": args.user_id.strip()}
    ...
    print(f"[install] wrote feed.json (user_id={args.user_id}, url={args.url})")
