...
- **身份信息**：`session_id`（唯一标识）、`runtime`（运行时类型）、`runtime_context.user_id`（用户标识）
...
1. **身份校验**：验证租户 Key 与 user_id，绑定租户内主体
...
4. **双写存储**：同时写入队列（`sessions/`）和归档（`session_archive/`），队列用于进化引擎消费，归档用于审计和历史回溯；进化引擎消费后排空归档同样保留 `value_judge` 与 `judge`（`team_skills/evolution/runtime/orchestrator.py:_archive_sessions`）
...
| `memory_candidate` | 有用的 Evidence 是用户特定偏好或习惯（而非团队 SOP），且对该用户未来任务可能持续有用 | 跳过并归档。ingest 阶段不存在 Memory 进化路由（DreamCycle 已被取代）：凡 decision 不为 `valuable` 的会话一律跳过，`memory_candidates` 仅作为判定结果的一部分随 `value_judge` 记录（`session_ingestion/service.py:ingest`） |
| `task_only` | 真实任务请求但尚无完成成果或可操作的进化 Evidence | 跳过并归档，等待更多 Evidence 累积 |
| `chitchat` | 社交、空对话或非任务交互 | 跳过，不进入进化 |

### 判定模式

仅支持合并大模型分析（`mode=merged`）：入库前必须得到价值分类、轨迹摘要和四维评分。用户反馈、Candidate 审计标记和历史分数都作为模型输入，不会直接决定分类或免除评分。

Trace 同样必须逐条取得模型结论，结果标记 `source=llm`。缺少模型、调用失败、漏评或输出不合法时，本次分析失败，不生成规则替代结果，不把缺失结论当作正常，也不写成已完成。长 Session 按批处理全部 Trace，全部批次成功后才触发结果钩子。

后续进化仅复用具有 `source=llm` 的完整评分、摘要和 `mode=merged` 分类。旧规则结果或仅有总分的记录，在重新接入或进入进化周期时必须重新分析。
...
合并分析器读取 Session 轨迹、交互、工具结果、可提取的源工件和输出工件，以及 Skill 使用等元数据。摘要由模型生成，与分类和评分在同一次调用中返回；三部分必须全部有效才提交结果。
...
- 指纹由每轮的 prompt_text、response_text、runtime 类型与用户主体、context_usage（snapshot_id、memory_refs、skill_refs、feedback）共同计算
