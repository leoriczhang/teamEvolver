...
1. **输入**：Checklist 项列表、交互历史（interactions）、工具轨迹（tool_trajectory）、workspace 产物文件列表（workspace_artifacts，含文本预览，最多 40 个文件）。本地分支的产物来自分支 workspace；服务端驱动的远程分支没有本地 workspace，Judge 收到的是 Agent 逐轮上报的 `artifacts`（映射为 `workspace_artifacts`，见 `team_replay/engine.py:_evaluate_local_checklist`）
...
| 模块 | 路径 |
|------|------|
| 渐进披露与 Checklist 决策 | [replay/policy.py](../../../team_replay/policy.py) |
| Checklist 项生成 | [dataset_synthesizer.py](../../../team_replay/datasets/synthesis.py) |
| 本地 Checklist Judge | [replay/engine.py](../../../team_replay/engine.py)（`_evaluate_local_checklist`） |
| 效率比较决策 | [replay/metrics.py](../../../team_replay/metrics.py) |
