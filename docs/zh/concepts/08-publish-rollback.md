> 本页 outbox/push 段落为迁移期历史机制。严格发布使用 [Skill pull](../api/06-skill-sync.md)：发布保留版本与 tombstone，不创建 delivery outbox。
...
| SkillHub（底层存储操作） | [skills/hub.py](../../../team_skills/library/hub.py) |
...
| Skill ID 注册表 | [skills/registry.py](../../../team_skills/library/registry.py) |
