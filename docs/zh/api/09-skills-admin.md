...
统一 ZIP、Skill 市场和 Git 导入导出 API 见 [Skill 导入导出](./18-skill-transfer.md)。控制台不再提供这些入口；API 仍沿用当前租户及 Skill 版本链。
...
Skill 编辑器：`team_skills/library/editor.py`
云端同步：`team_skills/library/mutations.py`（`SkillMutationService`）、`team_skills/library/hub.py`（`SkillHub`）
...
所有团队 Skill 写操作统一经过 `team_skills/library/mutations.py:SkillMutationService`（动作 `publish|update|rollback|delete`）。每次变更产生三类持久化记录：
