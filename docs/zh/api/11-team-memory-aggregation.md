...
> 本页保留聚合阶段的历史参数说明。当前两阶段执行、任务接口、Skill 冻结、失败恢复与配额限制以 [团队 Memory 两阶段进化](./12-team-memory.md) 为准。原有 `skill_revision` 请求字段及旧版 compile 上游接口已停用。
...
代码实现：
- 路由：`team_memory/routes.py`（`AggregationMixin`）
- 编排服务：`team_memory/service.py`（`MemoryAggregationService`）
- 确定性 staging：`team_memory/aggregation/staging.py`（`DeterministicStagingClient`）
- compile 调用：`team_memory/compile_client.py`（`CompileClient`）
- 用户枚举：`team_memory/aggregation/sources.py`（`AccountSourceBuilder`）
- 增量状态：`team_memory/aggregation/state.py`（`AggregationState`）
- OKF Skill 默认模板：`team_memory/aggregation/okf_skill.py`（`DEFAULT_OKF_SKILL_BODY`）

`CompileClient` 直接调用 OpenViking HTTP 接口：通过 `GET/POST/PUT /api/v1/skills`
读取或发布账号级共享 Skill，再通过 `POST /api/v1/compile` 创建任务并轮询
`GET /api/v1/tasks/{task_id}`。请求体固定为 `{from, to, skill}`（可选 `instruction`）；
当本次运行提供 `last_compile_time` 时，额外携带 `args.last_compile_time` 走增量编译。
teamEvolver 宿主机不需要安装 `ov` CLI，也不需要访问 OpenViking 容器内的
...
代码入口：`team_memory/routes.py` (`api_aggregation_users`)
...
代码入口：`team_memory/routes.py` (`api_aggregation_runs`)
...
| `last_compile_time` | string \| number | 否 | 增量编译起点，透传为 `ov compile` 的 `args.last_compile_time`。可为 ISO 时间字符串或 Unix 时间戳（秒）；不写时区按 UTC 处理，非法值直接 400。OpenViking 会递归列出各 `from` 来源并跨源去重，跳过 `modTime`（缺失时取 `mtime`）早于该时刻的文件；更新时间缺失或解析失败的文件保留以避免漏编，全部被过滤时任务直接成功结束、不调用模型、不写产物。该时间由调用方传入，系统不会自动记录或推进；不传保持全量编译 |

**响应（202）：** 初始 Run 对象（`status` 为 `pending`/`running`）。

`target_uri` 是运行级参数，不修改持久化设置。服务会按 Endpoint、Account、认证模式和目标 URI 隔离增量状态。`last_compile_time` 同样是运行级参数，仅作用于本次任务内的全部 merge / publish compile（不作用于 DreamCycle 维护 compile）。`root_key` 和 `admin_key` 都不会出现在 202 响应、任务列表或状态查询结果中。
...
代码入口：`team_memory/routes.py` (`api_aggregation_run`)
...
| `last_compile_time` | string | 本次任务规范化后的增量编译起点（ISO-8601）；未启用增量时为空串 |
...
代码入口：`team_memory/routes.py` (`api_aggregation_status`)
...
代码入口：`team_memory/routes.py` (`api_aggregation_okf_skill`)
...
代码入口：`team_memory/routes.py` (`api_aggregation_okf_skill_save`)
...
提交 `POST /api/v1/compile` 获得 `task_id` 后，通过鉴权请求轮询任务：
...
- `status`：任务状态；`completed`、`failed`、`cancelled` 为终态。
- `stage`：当前执行阶段。
- `result`：成功结果；失败时查看 `error.message` 或 `error.code`。
