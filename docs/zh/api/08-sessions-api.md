...
| 字段 | 类型 | 说明 |
|------|------|------|
| `summary.title` | string | 会话标题 |
| `summary.user_alias` | string | 用户别名 |
| `summary.status` | string | 处理状态 |
| `summary.num_turns` | integer | 轮次数 |
| `meta` | object | 适配器/推送提取的业务元信息（`user_id`、`session_id`、`trace_id`），与 `/conversations` 列表行同形 |
...
每个周期记录包含 `session_judge` 汇总（`enabled`、`judged_sessions`、`scored_sessions`、`mean_score`、`min_score`、`max_score`）与 `session_judge_details`（每个被消费 Session 的 `session_id`、`overall_score`、四个维度分数、各维度 `reasons` 与 `rationale`，`team_skills/evolution/runtime/orchestrator.py:_collect_session_judge_details`）。
