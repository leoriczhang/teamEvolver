...
新客户端直接使用 [租户 Key + user_id](../agent-integrations/06-protocol-v2.md)。
控制台已改为用户管理和租户 Replay 运行环境绑定。

兼容发布仍保留旧注册控制面，供未升级客户端迁移；这些接口不是 v2 的前置步骤。
阶段 6 独立清理发布删除旧注册、subject 映射及 push 管理接口。
历史请求格式见 [v1 迁移参考](../agent-integrations/02-protocol-v1.md)，执行顺序见
[升级指南](../guides/11-agent-deregistration.md)。

以下保留完整的 V1 注册参考，供兼容窗口内使用。

Agent 注册接口用于将新的 Agent 运行时注册到 teamEvolver，声明其身份（`agent_id` = integration_id）、`runtime_type`、能力（capabilities）、回调端点和元数据。V1 注册使用控制面密钥认证。注册**不再签发任何令牌**，它仍然是接入的必经步骤：注册记录是 `integration_id → runtime_type → 主体映射` 的唯一来源，也是 Replay capability 解析的唯一来源。

Agent 数据面现在只有**一个**机器凭证：**租户机器凭证**（格式 `tevt_<random>`）。多租户部署下由控制台在创建租户时签发（`POST /api/tenants`），仅展示一次，服务端只存储其 SHA-256 哈希，轮换接口为 `POST /api/tenants/{tenant_id}/rotate-token`（旧凭证立即失效）；单租户部署下由运维通过 `TEAMEVOLVER_TENANT_TOKEN` 环境变量配置。它是租户级全权机器凭证，不做 capability → scope 收敛，但调用 Context Workspace 时必须在请求中声明 `integration_id`；详见 [Context Workspace API](./04-context-workspace.md)。
...
响应中**不包含**任何凭证字段（注册不再签发令牌）：Agent 数据面使用租户机器凭证 `tevt_<random>`。
...
teamEvolver 侧需导出 `TEAMEVOLVER_AGENT_MY_AGENT_REPLAY_API_KEY=<secret>`，turn 服务端通过 `--api-key` 传入同一密钥。注册响应不包含任何凭证：注册只声明身份，Agent 数据面统一使用租户机器凭证。
...
4. 同一 agent_id 重复注册会更新该 Agent 的身份、能力与端点记录（不会签发或轮换任何令牌——注册不再签发令牌）。
5. Agent 数据面只有租户机器凭证（`tevt_`）一种机器凭证，它与注册流程相互独立：注册声明身份，凭证由控制台（多租户）或 `TEAMEVOLVER_TENANT_TOKEN`（单租户）提供。
