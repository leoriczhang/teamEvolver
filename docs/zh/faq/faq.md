...
1. 从管理员取得服务地址和租户机器凭证 `tevt_`。
2. 配置租户内的 `user_id`。
3. 在 Agent Session 结束后按 [v2 协议](../agent-integrations/06-protocol-v2.md) 调用 `/ingest_session`。

V1 协议下（兼容窗口）：

1. 使用 control-plane key 调用 `/internal/agents/register` 注册 Agent
2. 从运维获取并保存**租户机器凭证** `tevt_`（注册本身不再签发任何令牌）
...
使用租户 Key 与 user_id；详见 [v2 接入](../agent-integrations/06-protocol-v2.md)。旧映射错误仅出现在迁移期 v1 请求，应升级客户端。

### Agent 侧的凭证泄露了怎么办？

Agent 数据面只有一个机器凭证——**租户机器凭证（`tevt_`）**——因此 Agent 侧泄露的就是它。注册不再签发按 Agent 的令牌，也不存在按 Agent 的轮换方式。

- **多租户部署**：在控制台「租户管理」租户页调用 `POST /api/tenants/{tenant_id}/rotate-token` 轮换（需控制台身份或 Root Key）。新凭证签发后旧凭证立即失效，服务端只保存 SHA-256 哈希。
- **单租户部署**（未启用 `storage_pg`）：修改 `TEAMEVOLVER_TENANT_TOKEN` 环境变量的值并重启进程即可完成轮换。

该凭证只在签发时（单租户为配置时）完整展示一次，丢失后无法找回，只能重新签发或轮换。注意它以**租户**为粒度：轮换会影响该租户下所有 Agent。

### 可以接入多个 Agent Runtime 吗？

可以。不同 Runtime 使用同一租户 Key 并声明实际用户。Runtime 类型仅作为分析与运行环境选择标签，Context 和 Session 按 tenant/user 隔离。

V1 协议下（兼容窗口），每个 Agent Runtime 注册时获得独立的 `integration_id`（注册不再签发令牌，数据面统一使用租户机器凭证），彼此通过声明的 `integration_id` 归属隔离。teamEvolver 会聚合所有已注册 Agent 的 Session 用于进化。
