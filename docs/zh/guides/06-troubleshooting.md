...
3. 验证 Trusted Root Key 配置：
   - `sharing.viking_api_key`
   - 或兼容字段 `sharing.viking_team_api_key`
...
### 问题：Langfuse 拉取数量少于预览窗口

1. 查看响应中的 `total`、`counts.filtered` 和每条 filtered 结果的 `reason`。
2. 检查绑定 Adapter 的 `SOURCE.exclude_session_id_patterns`。匹配采用大小写敏感 glob，例如 `origin-*`。
3. 排查单条噪声 Session 时显式传 `session_id`；单 Session 请求会绕过自动排除。
4. 网络错误、超时、408、429 和 5xx 最多尝试 3 次。查看 `retry_count` 和 `[Langfuse] retrying GET` 日志；401/403、其他 4xx 与无效 JSON 不重试。

## 双 Pod 与发布链路问题

### 问题：不同 Pod 的 Skill 数量或候选状态不一致
...
1. 两侧 `tenant_id` 必须一致，`multi_replica_safe` 必须为 `true`。
2. `skill_storage.effective_backend` 与 `session_storage.effective_backend` 都应为 `postgres`。
3. 先比较 `manifest_hash`，再检查 `local_cache_matches_manifest`。manifest 相同但缓存不同时，等待 `sharing.skill_reload_interval_seconds` 后重试。
4. manifest 不同时暂停发布，确认所有 Pod 使用同一 PG DSN/schema，并执行本地控制面迁移，不能复制覆盖冲突对象。

### 问题：发布成功但 rollout 没有到达 upclaw

1. 查看 `/storage/status` 的 `outbox.backlog`、`dead_letter` 和 `last_error`。
2. 查看 `rollout_supervisor.running` 和租户状态。`lease_acquired=true` 表示当前实例上一轮取得租约；请求返回时 `lease_held` 应为 `false`。
3. payload/frontmatter 错误会直接 dead-letter，不会请求上游；400/401/403/404/409/422 默认不可重试。
4. 408、429、5xx 会保留 workspace 并重试。按 `event_id`、`workspace`、`skill_name` 和 `version` 关联日志。
5. 候选接口返回 503 表示 durable publish 未完成，不能把候选当成 published；修复存储后用同一 revision 重试。
...
经验库同步面板分别显示来源读取缺口、上传重试与索引待确认。查看状态接口的 `manual.import.error_counts` / `errors_sample`，并用来源标识查找 `experience_sync.import_source_rejected`；不要只看最后一个错误。默认 64 MiB 来源预算仅用于显式存量导入，单条投影最多 256 KiB。其他来源会继续处理，来源版本变化时未确认页面不会上传。

升级 TE 后，既有待同步文档按原退避自动恢复，不清理 PG 状态。旧导入结束后重新点击“导入存量成功经验”补齐先前跳过的来源。mkdir 的 409 必须通过同身份 stat 确认目录；同名文件、stat 鉴权或网络故障仍需处理。`OV_INDEX_NOT_READY` 不能当作全部同步成功。详见[成功经验同步运维指南](16-successful-experience-sync.md)。


### 73 条等待重试，看不到上传进度

“导入已准备”表示来源扫描/归并结束，不代表文件已同步。相同正文会沿用先前失败的退避（最长 15 分钟），不会因再次导入立即重试。查看页面的后台执行状态、最近上传检查、已到期/退避中数量、最早重试时间，以及错误发生时间。`CONFLICT` 可能来自历史状态，不能只凭显示的最后错误判断最新请求。

日志 `experience_sync.delivery_started` 标明开始尝试；`experience_sync.delivery` 标明结果、失败阶段、次数及下次时间。`experience_sync.delivery_pass` 报告批次尝试和退避跳过量；无变化的等待每分钟最多汇总一次。`experience_sync.blocked` 带安全错误类别和发生阶段；锁繁忙时页面显示租户锁被其他执行者持有。

若 write 约 25 秒后返回 `504 DEADLINE_EXCEEDED`，这是 TE 请求的 OV 处理等待期限；不能仅据此认定 TE 卡死或正文未落盘。TE 会回读同 URI，`experience_sync.write_reconciled` 记录是否确认正文；确认后仍需索引完成，页面显示“正文已写入、索引待确认”。用该次 OV request ID 查 OV 的语义/向量队列与模型日志，区分积压、调用失败或处理较慢；不要用下一条不同 URI 的 404 推断前一条写入失败。不通过无限增加超时或只看文件存在来宣告成功。

目标目录不再自动追加 TE 租户 ID；防重入使用现有 PG 租户 advisory lock，目录冻结不是执行锁。不同租户共用同一 OV 目标时，不提供跨租户的目录互斥。

本次诊断改动本地验证（2026-09-22，基线 `028f36b`）：全量 Python 807 通过、48 跳过，5 项既有失败（Dataset Collection 历史来源、Skill 验证队列、租户模型配置、SF Agent 标题、存储状态）；包含真实隔离 PG 与新增退避/超时回读契约用例。前端 74 通过，构建、编译、Ruff、文档检查及隔离前台/daemon 启动通过。首次 PG 验证因测试实例端口未沿用导致连接失败，纠正为 55493 后重新执行得到以上结果。未部署或触发 SIT 导入；真实 OV 队列超时原因仍需部署日志确认。
