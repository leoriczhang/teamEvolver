...
Skill Miner 是顶层深模块，代码统一位于 `team_miner/`。主服务通过 `team_miner/bridge.py` 接入 HTTP 路由，白盒配置和候选提交分别由 `team_miner/settings.py`、`team_miner/lifecycle.py` 管理；统一控制台视图位于 `team_miner/frontend/`，独立 Web 控制台位于 `team_miner/web_console/`。
...
- **Prompt**：由 `team_miner/sample_package_constructor_agent_prompt.py` 中的 `SAMPLE_PACKAGE_CONSTRUCTOR_AGENT_PROMPT` 定义
...
- **Prompt**：由 `team_miner/semantic_discovery_agent_prompt.py` 中的 `SEMANTIC_DISCOVERY_AGENT_PROMPT` 定义
- **输入**：Step 1 产出的样本包
...
- **Prompt**：由 `team_miner/evaluation_compiler_agent_prompt.py` 中的 `EVALUATION_COMPILER_AGENT_PROMPT` 定义
- **输入**：Step 2 的语义分析报告
...
首次运行会自动从 `team_miner/hermes/config.yaml.example` 创建项目本地 Hermes 配置 `.hermes_home/config.yaml`。
...
将同一主题的 Markdown 文档放入 `team_miner/data/input/` 目录。
...
- `benchmark.json`：`team-replay.dataset.v2` 格式 Test Dataset
- `BENCHMARK.md`：人工可读版
...
- `snapshots/build-N.json`：每次构建的 `team-replay.dataset.v2` 快照；兼容读取历史 JSONL
...
人工检查点逻辑位于 `team_miner/human_checkpoints.py`，测试位于 `team_miner/tests/test_skillminer_human_checkpoints.py`。
...
独立控制台代码位于 `team_miner/web_console/static/`（`index.html`、`app.js`、`styles.css`）。
...
Skill Miner 包含三个内置的挖掘 Agent Skill，位于 `team_miner/` 下：
...
`team_miner/bridge.py` 提供主服务与 Skill Miner 之间的 HTTP API 桥接，包括：
...
主服务的 `/api/mining/*` 路由通过此桥接层调用 `team_miner/lifecycle.py` 中的生命周期管理函数。
