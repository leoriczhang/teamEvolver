...
| `teamEvolver.log` | 前台与 Daemon 默认日志文件（自动轮转） |
...
| `--log-file PATH` | 前台与 Daemon 指定日志文件路径，默认 `~/.teamEvolver/teamEvolver.log` |
...
2. Daemon 模式下：
   - 获取启动锁，防止重复启动
   - 创建子进程，由子进程的统一日志组件管理文件及轮转；stdin/stdout 丢弃，stderr 保留平台告警
...
推送通过 `team_skills/library/mutations.py:SkillMutationService` 执行（`_push_via_mutations` 路径）：每个技能写入一条变更提交和同步发件箱事件，由后台 flusher 异步投递到 OpenViking。结果包含 `uploaded`、`skipped`、`filtered`、`submitted`（提交数量）以及事件 ID 列表（`event_ids`）。
...
文件日志配置、自动轮转及写盘降级见 [服务日志与 Ontology 排障](15-service-logging.md)。
