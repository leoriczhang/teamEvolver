...
No. teamEvolver does not execute user tasks, does not provide model inference, and does not manage tool calls. It is a control layer sitting on top of existing Agent Runtimes (Hermes, Pi, Codex, etc.), responsible for enabling continuous evolution of team capabilities.
...
This indicates the `external_subject` reported by the Agent is not mapped to a teamEvolver user. The administrator needs to add the mapping in the console "User Management" section, or batch sync mappings via the `subject_mappings` field during registration.

### What if the Agent Access Token is compromised?

Revoke the old token in the console or via API, then re-register or call the token rotation interface to obtain a new token. The system only stores the SHA-256 hash of tokens.
...
Default is `publish_mode: validated` + `human_review_enabled: true`, meaning human review is still required after passing automatic validation. You can set `human_review_enabled: false` to auto-publish Candidates that pass validation.
...
- **Team Memory:** Formed after DreamCycle deduplication, de-personalization, and aggregation; readable by all authorized users, written by the evolution process

## Troubleshooting

### After service startup, /status shows openviking_connected: false?

Check that `sharing.viking_endpoint` and `sharing.viking_api_key` are correct and the OpenViking service is network-reachable.
