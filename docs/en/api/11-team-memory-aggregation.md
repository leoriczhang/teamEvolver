...
The historical aggregation reference below is superseded by [Two-Stage Team Memory](./12-team-memory.md) for execution, Skill freezing, upstream endpoints, recovery, and full-checkout limits. The old `skill_revision` request field and legacy compile endpoints are no longer used.
...
Implementation:
- Routes: `team_memory/routes.py` (`AggregationMixin`)
- Orchestration: `team_memory/service.py` (`MemoryAggregationService`)
- Deterministic staging: `team_memory/aggregation/staging.py` (`DeterministicStagingClient`)
- Compile invocation: `team_memory/compile_client.py` (`CompileClient`)
- User enumeration: `team_memory/aggregation/sources.py` (`AccountSourceBuilder`)
- Incremental state: `team_memory/aggregation/state.py` (`AggregationState`)
- Default OKF Skill: `team_memory/aggregation/okf_skill.py` (`DEFAULT_OKF_SKILL_BODY`)

`CompileClient` calls OpenViking over HTTP. It reads or publishes the account-shared
Skill through `GET/POST/PUT /api/v1/skills`, then creates tasks through
`POST /api/v1/compile` and polls `GET /api/v1/tasks/{task_id}`. The request body
is `{from, to, skill}` (plus an optional `instruction`); when a run supplies
`last_compile_time` it also sends `args.last_compile_time` for an incremental compile.
...
Code entry: `team_memory/routes.py` (`api_aggregation_users`)
...
Code entry: `team_memory/routes.py` (`api_aggregation_runs`)
...
| `last_compile_time` | string \| number | No | Incremental-compile floor, forwarded to `ov compile` as `args.last_compile_time`. Accepts an ISO time string or a Unix timestamp (seconds); a naive time is treated as UTC and a malformed value returns 400. OpenViking recursively lists the `from` sources, de-duplicates files across them, and skips any file whose `modTime` (falling back to `mtime`) is earlier than this instant; files with a missing/unparseable update time are kept to avoid dropping edits, and if everything is filtered the task still succeeds without invoking the model or writing artifacts. The value is caller-supplied and never advanced automatically; omit it for a full compile |

**Response (202):** initial Run object (`status` is `pending`/`running`).

`target_uri` is scoped to this run and does not mutate persisted settings. Incremental state is isolated by endpoint, Account, authentication mode, and target URI. `last_compile_time` is likewise run-scoped and applies to every merge/publish compile in the run (not the DreamCycle maintenance compile). Neither credential appears in the 202 response, task list, or status response.
...
Code entry: `team_memory/routes.py` (`api_aggregation_run`)
...
| `last_compile_time` | string | Normalized incremental-compile floor (ISO-8601) for this run; empty string when incremental compile is off |
...
Code entry: `team_memory/routes.py` (`api_aggregation_status`)
...
Code entry: `team_memory/routes.py` (`api_aggregation_okf_skill`)
...
Code entry: `team_memory/routes.py` (`api_aggregation_okf_skill_save`)
...
### Compile task diagnostics

After `POST /api/v1/compile` returns a `task_id`, poll it with an authenticated
request:
...
curl -H "X-API-Key: <openviking-key>" \
  ...
  "https://openviking.example.com/api/v1/tasks/<task_id>"
```

Key fields:

- `status`: task state; `completed`, `failed`, and `cancelled` are terminal.
- `stage`: the current execution stage.
- `result`: successful output; on failure inspect `error.message` or `error.code`.
