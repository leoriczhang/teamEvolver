...
Skill Miner is a top-level deep module under `team_miner/`. The main service uses `team_miner/bridge.py` for HTTP integration, while `team_miner/settings.py` and `team_miner/lifecycle.py` own white-box settings and candidate submission. The unified-console views live under `team_miner/frontend/`, and the standalone Web console lives under `team_miner/web_console/`.
...
- **Prompt**: Defined by `SAMPLE_PACKAGE_CONSTRUCTOR_AGENT_PROMPT` in `team_miner/sample_package_constructor_agent_prompt.py`
...
- **Prompt**: Defined by `SEMANTIC_DISCOVERY_AGENT_PROMPT` in `team_miner/semantic_discovery_agent_prompt.py`
...
- **Prompt**: Defined by `EVALUATION_COMPILER_AGENT_PROMPT` in `team_miner/evaluation_compiler_agent_prompt.py`
...
First run automatically creates project-local Hermes config `.hermes_home/config.yaml` from `team_miner/hermes/config.yaml.example`.
...
# Or edit directly
${EDITOR:-vi} team_miner/.hermes_home/config.yaml
```
...
Place same-topic Markdown documents into the `team_miner/data/input/` directory.
...
- `benchmark.json`: `team-replay.dataset.v2` Test Dataset
...
- `snapshots/build-N.json`: `team-replay.dataset.v2` snapshots from each build; legacy JSONL remains readable
...
Human checkpoint logic lives at `team_miner/human_checkpoints.py`; its tests live at `team_miner/tests/test_skillminer_human_checkpoints.py`.
...
Standalone console code lives at `team_miner/web_console/static/` (`index.html`, `app.js`, `styles.css`).
...
Skill Miner includes three built-in mining Agent Skills under `team_miner/`:
...
`team_miner/bridge.py` provides the HTTP integration between the main service and Skill Miner, including:
...
Main service `/api/mining/*` routes call lifecycle functions in `team_miner/lifecycle.py` through this bridge.
