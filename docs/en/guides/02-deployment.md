...
teamEvolver configures Session and Skill control-plane storage independently. When `storage_pg.enabled=true`, unspecified Skill and Session backends both resolve to PostgreSQL; tenants cannot override these service-wide storage settings. Multi-replica deployments require both effective backends to be `postgres`. Each Pod's `skills.dir` is only a rebuildable cache.
...
| `sharing.skill_local_root` | empty → inherits `local_root` | Local/NAS Skill-storage root |
| `sharing.session_backend` | `postgres` with PG enabled; otherwise `local` | Session backend |
| `sharing.skill_backend` | `postgres` with PG enabled; otherwise `local` | Authoritative backend for Skills, candidates, reviews, versions, mutation commits, and outbox events |
...
Explicit `local` is suitable for one instance or a genuinely shared NAS, but lacks PostgreSQL transaction/CAS and cross-Pod leases. Writes made during OpenViking fallback stay local and are not automatically synced back. Inspect `GET /storage/status`: `skill_storage`, `session_storage`, `multi_replica_safe`, `pg`, `skill_cache`, `outbox`, and `rollout_supervisor`.
...
Single-tenant deployments (`storage_pg` disabled) also need the **only** machine credential on the Agent data plane—the tenant machine credential. It is supplied via the `TEAMEVOLVER_TENANT_TOKEN` environment variable (or `tenant.machine_token` in config.yaml), must start with `tevt_`, and the server resolves it to the `default` tenant:

```bash
# Generate the tenant machine credential
TEAMEVOLVER_TENANT_TOKEN="tevt_$(python3 -c 'import secrets;print(secrets.token_urlsafe(32))')"

# Set in systemd service file:
# Environment=TEAMEVOLVER_TENANT_TOKEN=$TEAMEVOLVER_TENANT_TOKEN
```

When it is not configured, the console's tenant page shows a hint and the generation command above; rotation means changing the value and restarting the process. Multi-tenant deployments do not need this variable: the credential is issued by the console at tenant creation and rotated on the tenant page.
...
Use this control-plane baseline for two Pods:

```yaml
storage_pg:
  enabled: true
sharing:
  skill_backend: postgres
...
skillopt_rollout:
  enabled: true
  endpoint: https://UPCLAW_ENDPOINT
```

PostgreSQL is authoritative for publication, candidates, versions, and outbox events. Every Pod refreshes its local Skill cache per tenant. The rollout supervisor re-reads tenant settings every tick and uses the PostgreSQL advisory lock `skillopt-rollout-v1:<tenant_id>` for single-consumer delivery. Before scaling out, migrate all Pod-local control-plane roots, require `multi_replica_safe=true`, and compare `manifest_hash` plus cache status through the load balancer.
