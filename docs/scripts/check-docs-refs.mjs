...
const mdFiles = [
  ...
  ...walkMarkdown(path.join(docsRoot, 'ontology-integration')),
]
...
const codeEntryRegex = /(?:^|\s)`((?:teamEvolver|team_memory)\/[a-zA-Z0-9_/.]+\.py)(?::([a-zA-Z_][a-zA-Z0-9_.]*))?`/gm

// Matches code-entry lines that look like: `- `teamEvolver/...``
const codeEntryListRegex = /`((?:teamEvolver|team_memory)\/[a-zA-Z0-9_/.]+\.py)`/g
...
// 6. Check local schema references. Repository-qualified paths in source manifests
// belong to the named external repository, not automatically to docs/schemas.
...
if (fs.existsSync(schemasDir)) {
  ...
  for (const mdFile of mdFiles) {
    const content = fs.readFileSync(mdFile, 'utf8')
    for (const m of content.matchAll(/(?<![a-z0-9_./-])(?:docs\/schemas\/|(?:\.\.\/)+schemas\/|schemas\/)?([a-z0-9_-]+\.schema\.json)/g)) {
      schemaRefs.add(m[1])
    }
  }
