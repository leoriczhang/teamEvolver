...
class AggregationState:
    ...
    def mark_failed(
        ...
        """Mark a group failed and count the attempt.

        The attempt counter bounds cross-run retries: a group whose inputs
        (same source fingerprint) keep failing is retried only a limited
        number of times before the caller abandons it, so a permanently
        failing compile cannot loop forever. A changed input fingerprint
        resets the counter because it represents a new problem.
        """
        entry = self.groups.get(group_key)
        ...
        if not source_fingerprint:
            source_fingerprint = str(failed.get("source_fingerprint") or "")
        attempts = failed.get("attempts")
        if (
            not isinstance(attempts, int)
            or attempts < 0
            or str(failed.get("source_fingerprint") or "") != source_fingerprint
        ):
            attempts = 0
        failed["attempts"] = attempts + 1
        failed["source_fingerprint"] = source_fingerprint
        self.groups[group_key] = failed

    def attempt_count(self, group_key: str) -> int:
        entry = self.groups.get(group_key)
        if not isinstance(entry, dict):
            return 0
    ...
    def attempts_exhausted(
        ...
        *,
        ...
        """True when a failed group exhausted its retry budget.

        ``source_fingerprint`` compares against the fingerprint recorded at
        failure time; pass ``None`` to judge by the attempt count alone
        (used when the current fingerprint is unknown). A group whose
        inputs changed since the failures is never considered exhausted.
        """
        entry = self.groups.get(group_key)
        if not isinstance(entry, dict):
            return False
        ...
        if (
            ...
            and str(entry.get("source_fingerprint") or "") != source_fingerprint
        ):
            return False
        return self.attempt_count(group_key) >= max(1, int(max_attempts))
