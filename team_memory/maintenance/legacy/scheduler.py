...
from team_memory.maintenance.legacy.config import DreamCycleConfig
...
from team_memory.maintenance.legacy.tools.base import ToolRegistry
...
if TYPE_CHECKING:
    from team_memory.maintenance.legacy.blackboard import Blackboard
...
class Scheduler:
    ...
    def _build_tools(self, blackboard: "Blackboard | None" = None) -> ToolRegistry:
        ...
        from team_memory.maintenance.legacy.tools.blackboard_tool import BlackboardTool
    ...
    def run_round(self) -> List[JobResult]:
        ...
        with langfuse_observation(
            name="team_memory.maintenance.legacy.round",
            as_type="agent",
            input={
                "run_id": run_id,
            ...
            metadata={
                "component": "team_memory.maintenance.legacy",
                "operation": "round",
                "run_id": run_id,
            },
            trace_name="team_memory.maintenance.legacy.round",
            session_id=run_id,
    ...
    def _run_round(self, run_id: str) -> List[JobResult]:
        ...
        for job_class in self._job_classes:
            ...
            try:
                with langfuse_observation(
                    name=f"team_memory.maintenance.legacy.job.{job.name}",
                    as_type="agent",
                    input={
                        "job": job.name,
