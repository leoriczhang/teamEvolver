...
from team_memory.maintenance.integrations.dreamcycle import collect_personal_source_keys, collect_personal_source_users, parse_openviking_key
...
class NativeDreamCycleSupervisor:
    ...
    async def _evolve(self, sources: list[dict[str, str]], existing: str) -> str:
        llm = AsyncLLMClient(
            api_key=str(getattr(self.config, "llm_api_key", "")),
            base_url=str(getattr(self.config, "llm_api_base", "")),
            model=str(getattr(self.config, "llm_model_id", "")),
            max_tokens=16384,
            temperature=0.2,
            tenant_id=current_tenant_id(),
            max_concurrency=int(
                getattr(self.config, "llm_max_concurrency", 8) or 8
            ),
            queue_capacity=int(
                getattr(self.config, "llm_queue_capacity", 64) or 64
            ),
        )
        ...
        candidates = await llm.chat([{"role": "system", "content": extract_prompt}, {"role": "user", "content": json.dumps(sources, ensure_ascii=False)}], trace_name="team_memory.maintenance.legacy.extract")
        return await llm.chat([{"role": "system", "content": consolidate_prompt}, {"role": "user", "content": f"## Existing team memory\n{existing}\n\n## Candidates\n{candidates}"}], trace_name="team_memory.maintenance.legacy.consolidate")
