...
from team_memory.maintenance.integrations.dreamcycle import (
    collect_personal_source_keys,
    collect_personal_source_users,
    parse_openviking_key,
)
from team_memory.maintenance.legacy.config import (
    DreamCycleConfig,
    LLMConfig,
    LogConfig,
    OpenVikingConfig,
...
from team_memory.maintenance.legacy.jobs import ALL_JOBS
from team_memory.maintenance.legacy.logging_config import setup_embedded_logging
from team_memory.maintenance.legacy.scheduler import Scheduler
from team_memory.memory_changes import MemoryChangeLedger
...
class FullDreamCycleSupervisor:
    ...
    def _missing(self) -> list[str]:
        values = {
            ...
            "llm.api_key": getattr(self.config, "llm_api_key", ""),
            "llm.model": getattr(self.config, "llm_model_id", ""),
        }
        return [
            key
    ...
    def _build_scheduler(self) -> Scheduler:
        ...
        dreamcycle_config = DreamCycleConfig(
            ...
            llm=LLMConfig(
                base_url=str(
                    getattr(self.config, "llm_api_base", "") or ""
                ),
                api_key=str(
                    getattr(self.config, "llm_api_key", "") or ""
                ),
                model=str(
                    getattr(self.config, "llm_model_id", "") or ""
                ),
                max_tokens=max(
                    1,
                    int(
                        getattr(self.config, "llm_max_tokens", 4096)
                        or 4096
                    ),
                ),
                temperature=max(
                    ...
                    min(
                        2.0,
                        float(
                            getattr(self.config, "llm_temperature", 0.3)
                            or 0.3
                        ),
                    ),
                ),
    ...
    def reset(
        ...
        *,
        ...
        from team_memory.maintenance.legacy.reset import reset
        ...
        if not dry_run:
            from team_memory.maintenance.legacy.scheduler import ExecutionState
    ...
    def _default_job_runtime(self) -> dict[str, Any]:
        return {
            ...
            "base_url": "",
            "temperature": float(
                getattr(self.config, "llm_temperature", 0.3) or 0.3
            ),
            "max_tokens": int(
                getattr(self.config, "llm_max_tokens", 4096) or 4096
            ),
            "max_turns": int(
                getattr(
                    self.config,
