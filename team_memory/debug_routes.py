...
def _entries(result: Any) -> list[dict[str, Any]]:
    ...
    def visit(value: Any) -> None:
        if isinstance(value, list):
            ...
            if value.get("uri") or value.get("path"):
                found.append(value)
...
class MemoryDebugMixin:
    ...
    def _register_memory_debug_routes(self, app: FastAPI) -> None:
        from teamEvolver.proxy.openviking_workspace import _OpenVikingRequestError, _validate_uri
        ...
        async def debug_memory_context(request: Request):
            ...
            if not query or len(query) > 8000:
                raise HTTPException(400, "invalid memory query")
            try:
            ...
            async def search(user, scope):
                try:
                    result = await owner._workspace_request(
                        ...
                        json={"query": query, "target_uri": scope.root_uri, "limit": max_items},
                    )
            ...
            for (_user, scope), matches in zip(resolved, results):
                for match in matches:
                    if len(items) >= max_items or used_chars >= max_chars:
                        break
                    try:
                        uri = _validate_uri(scope, match.get("uri") or match.get("path"), allow_root=False)
                    ...
                    items.append(
                        {
                            ...
                            "space": scope.space,
                            "kind": "memory",
                            "title": str(match.get("name") or match.get("title") or uri.rsplit("/", 1)[-1]),
                            "path_alias": alias,
                            "l0": l0,
                            "l1": _text(match.get("overview"), 4000),
                            "score": match.get("score"),
                            "selected": True,
                            "provenance": {"space": scope.space},
                        }
                    )
            rendered_parts = []
            for item in items:
                content = item["l0"] or item["l1"] or "（无可注入摘要）"
                rendered_parts.append(
                    f"## {item['title']}\nscope: {item['scope']}\n"
                    f"path: {item['path_alias']}\n\n{content}"
                )
            rendered = "\n\n".join(rendered_parts)
            return JSONResponse(
                content={
                    "schema_version": "teamevolver.memory-debug.v1",
                    "user_id": user_id,
                    "query": query,
                    "scopes": ["personal_memory", "team_memory"],
                    "items": items,
                    "agent_context": rendered,
                    "budget": {
                        "max_items": max_items,
                        "max_chars": max_chars,
