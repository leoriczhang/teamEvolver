...
from team_memory.maintenance.integrations import dreamcycle as dreamcycle_integration
...
def test_unified_dreamcycle_routes_are_available(tmp_path, monkeypatch) -> None:
    ...
    assert status.json()["engine"] == "team-memory-compile"
    assert status.json()["full_capabilities"] is False
...
def test_agentshub_config_sync_retires_personal_sources(
    tmp_path,
    monkeypatch,
) -> None:
    monkeypatch.setenv("TEAMEVOLVER_EMBEDDED_EVOLVE_ENABLED", "0")
    ...
    assert "personal_source_count" not in response.json()
    ...
    assert saved["viking_personal_api_keys"] == []
...
def test_v1_registration_syncs_existing_subject_mappings(
    ...
    assert response.status_code == 200
    # Registration only declares identity; the tenant machine credential is the
    # single Agent data-plane credential.
...
def test_native_memory_evolution_settings_round_trip(tmp_path, monkeypatch) -> None:
    ...
    assert dry_run.json()["jobs"] == []
    assert dry_run.json()["stages"] == ["snapshot", "maintenance"]
    assert dry_run.json()["maintained_space"].startswith("viking://resources/")
    ...
    initial = client.get("/api/skill-evolution/settings").json()["memory_maintenance"]
