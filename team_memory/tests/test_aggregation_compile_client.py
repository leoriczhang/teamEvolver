...
def test_http_transport_uploads_inline_skill_and_runs_compile_without_host_cli(
    ...
    class FakeAsyncClient:
        ...
        async def post(self, url, *, json, headers):
            ...
            assert url.endswith("/api/v1/compile")
            return _FakeResponse(
                202,
                {
                    "status": "ok",
    ...
    assert calls[1][1] == "http://openviking.example/api/v1/compile"
    assert calls[1][2]["from"] == [
        "viking://user/alice/memories/events"
    ]
    assert "skill_revision" not in calls[1][2]
    assert calls[2][1] == "http://openviking.example/api/v1/tasks/cmp_1"
...
def _capture_compile_client(monkeypatch, calls: list) -> CompileClient:
    class FakeAsyncClient:
        def __init__(self, *args, **kwargs):
            pass
        ...
        async def post(self, url, *, json, headers):
            calls.append(("POST", url, json, headers))
            return _FakeResponse(
                202, {"status": "ok", "result": {"task_id": "cmp_lct", "status": "accepted"}}
            )
...
def test_run_batch_forwards_last_compile_time_as_args(monkeypatch) -> None:
    calls: list = []
    client = _capture_compile_client(monkeypatch, calls)

    compiled = asyncio.run(
        client.run_batch(
            source_uris=["viking://resources/product-raw-document"],
            target_uri="viking://resources/wiki-0916",
            skill_uri="viking://agent/skills/llm-wiki",
            last_compile_time="2026-09-17T08:00:00+08:00",
        )
    )

    assert compiled["ok"] is True
    post_body = next(body for method, _url, body, _h in calls if method == "POST")
    # ISO input with an explicit offset is preserved (normalized to that offset).
    assert post_body["args"] == {"last_compile_time": "2026-09-17T08:00:00+08:00"}


def test_run_batch_normalizes_unix_timestamp_and_naive_time(monkeypatch) -> None:
    calls: list = []
    client = _capture_compile_client(monkeypatch, calls)

    asyncio.run(
        client.run_batch(
            source_uris=["viking://resources/docs"],
            target_uri="viking://resources/wiki",
            skill_uri="viking://agent/skills/wiki",
            last_compile_time=1758067200,
        )
    )
    unix_body = next(body for method, _url, body, _h in calls if method == "POST")
    # Unix seconds -> UTC ISO.
    assert unix_body["args"]["last_compile_time"] == "2025-09-17T00:00:00+00:00"
    ...
    asyncio.run(
        client.run_batch(
            ...
            target_uri="viking://resources/wiki",
            skill_uri="viking://agent/skills/wiki",
            last_compile_time="2026-09-17T08:00:00",
        )
    )
    naive_body = next(body for method, _url, body, _h in calls if method == "POST")
    # A naive datetime is interpreted as UTC.
    assert naive_body["args"]["last_compile_time"] == "2026-09-17T08:00:00+00:00"


def test_run_batch_omits_args_when_no_last_compile_time(monkeypatch) -> None:
    calls: list = []
    client = _capture_compile_client(monkeypatch, calls)
    ...
    body = next(body for method, _url, body, _h in calls if method == "POST")
    assert "args" not in body


def test_run_batch_rejects_malformed_last_compile_time_before_submit(monkeypatch) -> None:
    calls: list = []
    client = _capture_compile_client(monkeypatch, calls)
    ...
    result = asyncio.run(
        client.run_batch(
            ...
            target_uri="viking://resources/wiki",
            skill_uri="viking://agent/skills/wiki",
            last_compile_time="not-a-time",
        )
    )

    assert result["ok"] is False
    assert "last_compile_time" in result["stderr"]
    # No compile task should be created when the timestamp is invalid.
    assert not any(method == "POST" for method, _url, _b, _h in calls)


def test_normalize_last_compile_time_rejects_bool_and_empty() -> None:
    import pytest

    from team_memory.compile_client import normalize_last_compile_time

    with pytest.raises(ValueError):
        normalize_last_compile_time(True)
    with pytest.raises(ValueError):
        normalize_last_compile_time("   ")
    with pytest.raises(ValueError):
        normalize_last_compile_time(None)
