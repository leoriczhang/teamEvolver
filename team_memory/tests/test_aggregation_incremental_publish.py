...
from team_memory.service import (
...
def test_last_compile_time_flows_into_compile_calls(tmp_path) -> None:
    service = _service(tmp_path, batch_users=2)
    target_uri = "viking://resources/team-memory"
    work_root = service._work_root(target_uri)
    state_path = service._state_path("default", target_uri, "http://openviking.example")
    roots = _seed_state(service, state_path, target_uri, work_root, 3)
    ...
    assert calls, "expected at least one compile"
    # Every batch compile carries the run's normalized last_compile_time.
    assert all(
        c["last_compile_time"] == "2026-09-17T08:00:00+08:00" for c in calls
    )


def test_new_run_rejects_malformed_last_compile_time(tmp_path) -> None:
    import pytest
...
def test_new_run_without_last_compile_time_stays_full(tmp_path) -> None:
    service = _service(tmp_path)
    run = service.new_run(
        "default",
        endpoint="http://openviking.example",
    ...
    # Default keeps the full compile; nothing is forwarded to ov compile args.
    assert run.last_compile_time == ""
