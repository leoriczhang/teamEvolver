...
from team_memory import MemoryAggregationService
from team_memory.service import RunConflictError
...
def _request_user(request: Request) -> dict:
    # Resolve lazily so importing Memory routes does not bootstrap ProxyServer.
    from teamEvolver.proxy.users_admin import _request_user as resolve_user
...
class AggregationMixin:
    ...
    def _aggregation_service(self) -> MemoryAggregationService:
        ...
        elif getattr(service, "config", None) is not self.config and not service._active_targets:
    ...
    def _aggregation_request_context(
        ...
        if root_key:
            ...
            if endpoint != self._aggregation_service().resolve_endpoint():
                raise HTTPException(status_code=400, detail="A different endpoint requires an explicit credential")
            api_key = str(
    ...
    def _aggregation_settings_payload(agg: dict) -> dict:
        ...
        return {
            ...
            "maintenance_skill_uri": str(
                agg.get("maintenance_skill_uri") or "viking://agent/skills/team-memory-maintenance"
            ),
    ...
    def _register_aggregation_routes(self, app: FastAPI) -> None:
        ...
        async def api_aggregation_run(request: Request):
            ...
            if "user_ids" in body and (not isinstance(user_ids, list) or not user_ids):
                raise HTTPException(status_code=400, detail="user_ids must be a non-empty array or omitted")
            user_ids = (
            ...
            # Optional incremental compile: only sources modified at/after this
            # instant are recompiled. Accepts an ISO string or a Unix timestamp;
            # the value is caller-supplied and never advanced automatically.
            raw_last_compile_time = body.get("last_compile_time")
            if raw_last_compile_time is not None and not isinstance(
                raw_last_compile_time, (str, int, float)
            ):
                raise HTTPException(
                    status_code=400,
                    detail="last_compile_time must be an ISO string or Unix timestamp",
                )
            last_compile_time = (
                "" if raw_last_compile_time is None else str(raw_last_compile_time).strip()
            )
            ...
            try:
                run = service.new_run(
                    ...
                    pipeline=str(body.get("pipeline") or "both"),
            ...
            try:
                await service.reconcile(run, context.api_key)
                await asyncio.to_thread(service.submit, run, api_key=context.api_key,
                                        kinds=kinds, full=full, user_ids=user_ids)
            except RunConflictError as exc:
                service.forget_run(run.task_id)
                raise HTTPException(status_code=409, detail=str(exc)) from exc
            except Exception as exc:
                service.forget_run(run.task_id)
                raise HTTPException(status_code=503, detail="Cannot confirm upstream task state") from exc
        ...
        async def api_aggregation_okf_skill(request: Request, stage: str = "aggregation", account_id: str = ""):
            self._mark_request_activity()
            self._require_admin(request)
            if stage not in {"aggregation", "maintenance"}:
                raise HTTPException(status_code=400, detail="invalid Skill stage")
            service = self._aggregation_service()
            context = self._aggregation_request_context(request, {"account_id": account_id})
            ...
            return JSONResponse(
                {
                    "skill_name": service._skill_name(stage),
                    "skill_uri": service._shared_skill_uri(stage),
                    "revision": str((shared or {}).get("revision") or ""),
                    "body": str((shared or {}).get("content") or service.skill_body(stage)),
        ...
        async def api_aggregation_okf_skill_save(request: Request, stage: str = "aggregation", account_id: str = ""):
            self._mark_request_activity()
            self._require_admin(request)
            if stage not in {"aggregation", "maintenance"}:
                raise HTTPException(status_code=400, detail="invalid Skill stage")
            ...
            if not isinstance(payload, dict):
                raise HTTPException(status_code=400, detail="Skill body must be an object")
        ...
        async def api_aggregation_settings_save(request: Request):
            self._mark_request_activity()
            self._require_admin(request)
            if self._aggregation_service()._active_targets:
                raise HTTPException(status_code=409, detail="Team Memory tasks are still active")
            try:
                body = await request.json()
            except ValueError:
                body = {}
            ...
            for key in ("okf_skill_uri", "maintenance_skill_uri"):
                if key in body:
                    value = str(body.get(key) or "").strip().rstrip("/")
                    name = value.removeprefix("viking://agent/skills/")
                    try:
                        MemoryAggregationService._safe_path_segment(name, field_name=key)
                    except ValueError as exc:
                        raise HTTPException(status_code=400, detail=str(exc)) from exc
                    agg[key] = f"viking://agent/skills/{name}"
            if agg.get("okf_skill_uri") == agg.get("maintenance_skill_uri"):
                raise HTTPException(status_code=400, detail="The two stages require distinct Skills")
