...
import { Check, Copy, KeyRound, Plus, RefreshCw, Settings2 } from "lucide-react";

// Token reveal box: the plaintext machine credential is returned exactly once by
  ...
  return (
    <div className="rounded-[12px] border border-amber-300 bg-amber-50 p-4">
      ...
      <div className="mt-1 text-[11px] leading-relaxed text-amber-800">
        该 token 仅此一次完整展示，服务端只保留其哈希；请立即复制并妥善保管。它是本租户唯一的全权机器凭证，用于
        Agent 侧 <span className="mono"> Authorization: Bearer </span> 接入：Session 上报、Skill 拉取与
        Context Workspace；请求中需声明
        <span className="mono"> user_id </span>，Account 由租户配置确定。
      </div>
...
// Tenant governance owns lifecycle and quotas.
...
export default function TenantsView({
  ...
  const configured = Boolean(resp?.machine_credential?.configured);
  const envVar = resp?.machine_credential?.env_var || "TEAMEVOLVER_TENANT_TOKEN";
  const prefix = resp?.machine_credential?.prefix || "tevt_";
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setReveal({
        ...
        label: `租户「${data.tenant.display_name}」的机器凭证（全权）`,
        token: data.agent_token,
      });
      const ov = data.openviking_account;
      const ovAction = ov?.action;
      const ovLabel =
        ovAction === "created"
          ? `OpenViking 账号「${ov.account_id}」已创建并绑定`
          : ovAction === "bound"
            ? `OpenViking 账号「${ov.account_id}」已存在，自动绑定`
            : `OpenViking 账号绑定失败${ov?.error ? `：${ov.error}` : "，请手动创建同名账号"}`;
      const ovToast = ovAction === "failed" ? toastErr : toastOk;
      ovToast(ovAction === "failed" ? "账号绑定失败" : "账号已绑定", ovLabel);
  ...
  async function handleRotate(tenant: TenantInfo) {
    if (
      !window.confirm(
        `确定轮换租户「${tenant.display_name}」的机器凭证？旧 token 立即失效，使用它的 Agent 将无法再上报数据或读取 Context。`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setReveal({
        ...
        label: `租户「${tenant.display_name}」的新机器凭证（全权）`,
        token: data.agent_token,
      });
    ...
    <div className="space-y-4">
      {mode === "single" && (
        <div className="rounded-[14px] border border-border bg-surface p-4 text-[12px] leading-relaxed text-muted-foreground shadow-[var(--shadow-soft)]">
          ...
          未启用）：只有 default 一个租户，创建/禁用/轮换等管理操作不可用。接入凭证为
          <span className="mono"> {envVar} </span>
          配置的全权机器凭证：
          {configured ? (
            <b className="text-foreground"> 已配置</b>
          ) : (
            <>
              <b className="text-foreground"> 未配置</b>，请生成后写入环境变量并重启：
              <code className="mono mt-1 block break-all rounded bg-muted px-2 py-1 text-[11px]">
                {envVar}=&quot;{prefix}$(python3 -c &apos;import secrets;print(secrets.token_urlsafe(32))&apos;)&quot;
              </code>
            </>
          )}
          开启 PostgreSQL 后端后此页自动切换为多租户管理。
        </div>
      )}
      ...
      {mode === "postgres" && (
        ...
        >
          <div className="flex flex-wrap items-center gap-3 px-[14px] py-3">
            <Input aria-label="Account ID" value={accountId} placeholder="Account ID（可选，留空自动生成）" onChange={e => setAccountId(e.target.value)} className="max-w-[260px]" />
            <Input
              value={displayName}
              placeholder="项目名称"
            ...
            <span className="text-[11px] text-muted-foreground">
              创建后自动生成 <span className="mono">tevt_</span> 前缀的全权机器凭证，可用于 Session 上报、数据源拉取与 Context Workspace。
            </span>
          </div>
          <div className="px-[14px] pb-3 text-[11px] text-muted-foreground">
            Account ID 即租户标识 <span className="mono">tenant_id</span>，需与 OpenViking 中已存在的同名 account 一一对应（PG 多租户模式下，系统将以此值作为 <span className="mono">X-OpenViking-Account</span> 访问对应工作区）。留空时自动生成 <span className="mono">t_</span> 前缀的随机 ID，此时请确保 OpenViking 侧已有同名账号，否则该租户的 Viking 数据访问会失败。
          </div>
        </Panel>
      )}
                      ...
                      <div className="flex justify-end gap-2">
                        ...
                        <Button
                          ...
                          disabled={
                            mode !== "postgres" ||
                            t.tenant_id === "default" ||
                            busyTenantId === t.tenant_id
                          }
                          onClick={() => handleRotate(t)}
                          title={
                            t.tenant_id === "default"
                              ? "default 租户不签发机器凭证"
                              : "轮换全权机器凭证（旧 token 立即失效）"
                          }
                        >
