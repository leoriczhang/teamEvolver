...
export default function SkillLabView({
  ...
  onRunChange?: (run: SkillLabRun | null) => void;
  runSignal?: number;
}) {
  ...
  const externalDraftRef = useRef(externalDraftMd);
  externalDraftRef.current = externalDraftMd;
  ...
  const loadSkill = useCallback(async (name: string) => {
    if (!name) {
      setSkill(null);
      if (externalDraftRef.current == null) setDraftMd("");
      return;
    }
    const detail = await api<SkillDetail>(`/api/replay-lab/skills/${encodeURIComponent(name)}`);
    setSkill(detail);
    ...
    if (externalDraftRef.current == null) setDraftMd(detail.skill_md || "");
  }, []);
  ...
  const loadWorkspace = useCallback(async (name: string) => {
    ...
    const [datasetResp, runResp] = await Promise.all([
      api<{ datasets: SkillLabDataset[] }>(
        `/api/replay-lab/datasets?skill_name=${encodeURIComponent(name)}`
      ),
      api<{ runs: SkillLabRun[] }>(
        `/api/replay-lab/runs?skill_name=${encodeURIComponent(name)}&limit=100`
      ),
    ]);
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const skillResp = await api<SkillListResp>("/api/replay-lab/skills");
  ...
  useEffect(() => {
    if (!lockedSkillName || !active || lockedSkillName === skillName) return;
    ...
    const timer = window.setInterval(async () => {
      try {
        const response = await api<{ runs: SkillLabRun[] }>(
          `/api/replay-lab/runs?skill_name=${encodeURIComponent(skillName)}&limit=100`
        );
        setRuns(response.runs || []);
        if (activeRun?.run_id) {
          const detail = await api<SkillLabRun>(
            `/api/replay-lab/runs/${encodeURIComponent(activeRun.run_id)}`
          );
          setActiveRun(detail);
        }
      } catch {
  ...
  async function saveSkill() {
    ...
    setSavingSkill(true);
    try {
      await api(`/api/replay-lab/skills/${encodeURIComponent(skillName)}/save`, {
        method: "POST",
  ...
  function createDataset() {
    setDatasetDraft({ ...EMPTY_DATASET, skill_ids: skillName ? [skillName] : [] });
    setDatasetFiles([]);
  ...
  function editDataset(dataset: SkillLabDataset) {
    setDatasetDraft({
      ...
      skill_ids: dataset.skill_ids?.length ? dataset.skill_ids : (skillName ? [skillName] : []),
      name: dataset.name || "",
      query: dataset.query || "",
  ...
  async function saveDataset() {
    if (!datasetDraft.skill_ids.length || !datasetDraft.query.trim()) {
      toastErr("请选择至少一个 Skill 并填写实验 Query");
      return;
    }
    ...
    try {
      ...
      const saved = await api<SkillLabDataset>("/api/replay-lab/datasets", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function cloneDataset(dataset: SkillLabDataset) {
    try {
      const cloned = await api<SkillLabDataset>(
        `/api/replay-lab/datasets/${encodeURIComponent(dataset.dataset_id)}/clone`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function deleteDataset(dataset: SkillLabDataset) {
    ...
    try {
      await api(
        `/api/replay-lab/datasets/${encodeURIComponent(dataset.dataset_id)}?skill_name=${encodeURIComponent(skillName)}`,
        { method: "DELETE" }
      );
      toastOk("数据集已删除");
      await loadWorkspace(skillName);
  ...
  async function startRun() {
    ...
    setRunning(true);
    try {
      const run = await api<SkillLabRun>("/api/replay-lab/runs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          skill_name: skillName,
  ...
  async function synthesizeDatasets() {
    ...
    try {
      ...
      const result = await api<{ datasets: SkillLabDataset[]; count: number; source_session_count: number }>(
        "/api/replay-lab/datasets/synthesize",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  async function openRun(runId: string) {
    try {
      const detail = await api<SkillLabRun>(
        `/api/replay-lab/runs/${encodeURIComponent(runId)}`
      );
      setActiveRun(detail);
    } catch (error: any) {
  ...
  useEffect(() => { onRunChange?.(activeRun); }, [activeRun, onRunChange]);

  useEffect(() => {
    if (runSignal > 0 && active && !running && skill) void startRun();
  }, [runSignal]);
  ...
  return (
    <div className={cn("skill-lab-view min-w-0", compact && "skill-lab-compact", embedded ? "px-1 py-1" : "mx-auto max-w-[1440px] px-[22px] py-[22px]")}>
      {!compact && <div className="mb-5 grid grid-cols-[repeat(auto-fit,minmax(160px,1fr))] gap-3.5">
      ...
      <Panel
        title={compact ? "运行与调试" : "实验对象"}
        count={compact ? skillName : "Candidate 仅在实验中生效"}
        extra={
      ...
      {compact && (
        <div className="lab-quick-actions">
          <p>当前 Skill 的未保存文件一起参与 Candidate 运行。</p>
          <Button variant="outline" size="sm" disabled={!skillName} onClick={createDataset}><Plus className="size-3.5" />新建数据集</Button>
          {selectedDataset && <Button variant="ghost" size="sm" onClick={() => editDataset(selectedDataset)}>编辑数据集</Button>}
          {selectedDataset && <p className="lab-query-preview">{selectedDataset.query}</p>}
        </div>
      )}
      {!compact && <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(420px,0.95fr)]">
        <Panel
          ...
          >
            <div className="flex flex-wrap items-center gap-2 border-b border-line bg-muted/30 px-4 py-2.5">
              ...
              <Input
                ...
                placeholder="填入已入库的 Session ID（逗号分隔），留空则用近期会话"
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {!!dataset.skill_ids?.length && (
                                <Pill tone="gray">{dataset.skill_ids.length} 个 Skills</Pill>
                              )}
                              {(dataset.task_count || 1) > 1 && (
                                <Pill tone="gray">{dataset.task_count} 个任务</Pill>
                              )}
                              <Pill tone="gray">
      ...
      >
        <DialogContent className="flex max-h-[90vh] w-full !max-w-[860px] flex-col overflow-hidden p-0">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
            ...
            <Field label="关联 Skills *" hint="Skills 均为并列关系；当前任务可同时使用所选 Skills。">
              <div className="grid gap-2 sm:grid-cols-2">
                {skills.map((item) => {
                  const checked = datasetDraft.skill_ids.includes(item.name);
                  return (
                    <label
                      key={item.name}
                      className="flex cursor-pointer items-center gap-2 rounded-lg border border-border bg-background p-3 text-sm"
                    >
                      <input
                        type="checkbox"
                        className="size-4"
                        ...
                        onChange={(event) => setDatasetDraft({
                          ...
                          skill_ids: event.target.checked
                            ? [...datasetDraft.skill_ids, item.name]
                            : datasetDraft.skill_ids.filter((name) => name !== item.name),
                        })}
                      />
      ...
      {compact ? (
        <section className="lab-run-list">
          <h3>运行记录 <span>{runs.length}</span></h3>
          {!runs.length && <p>选择数据集后点击运行，Trace 将显示在下方控制台。</p>}
          {runs.map((run) => <button type="button" key={run.run_id} className={cn(activeRun?.run_id === run.run_id && "is-active")} onClick={() => void openRun(run.run_id)}>
            <span><RunStatus run={run} /><span className="truncate">{run.dataset_name || run.dataset_id}</span></span>
            <small>{fmtTime(run.created_at)} <Verdict value={run.result_summary?.verdict} /></small>
          </button>)}
        </section>
      ...
      {!compact && activeRun && <RunDetail run={activeRun} onRefresh={() => openRun(activeRun.run_id)} />}
    </div>
  );
}
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {!!dataset.skill_ids?.length && (
            <Pill tone="gray">Skills: {dataset.skill_ids.join("、")}</Pill>
          )}
          {(dataset.task_count || 1) > 1 && (
            <Pill tone="gray">{dataset.task_count} 个任务</Pill>
          )}
...
export function RunDetail({ run, onRefresh }: { run: SkillLabRun; onRefresh: () => void }) {
