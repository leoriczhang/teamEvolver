...
class MemoryReplayLedger(Protocol):
...
class ReplaySessionStore(Protocol):
...
class MemoryTrueReplayRunner:
    ...
    def __init__(
        ...
        *,
        ...
        session_store: ReplaySessionStore | None = None,
    ) -> None:
        self._ledger = ledger
    ...
    def _execute_replay(
        ...
        *,
        ...
        runtime_type = source_runtime_type(session)
        factory = resolve_factory(session) if self._branch_runner is spawn_native_agent_branch else None
        ...
        if factory is not None:
            replay_contexts = {
                branch: make_context(branch, None, common_case, session, timeout, snapshot=contexts[branch])
                for branch in ("baseline", "candidate")
            }
            results = execute_pair(factory, replay_contexts, common_case,
                                   harness=read_team_evolver_harness(), max_interactions=interactions)
        else:
    ...
    def _select_source_session(self, session_id: str) -> dict[str, Any]:
        ...
        host = current_host(required=store is None)
        if wanted:
            session = (
                store.load_session(wanted)
                if store is not None
                else host.load_source_session(wanted)
            )
            if session is None:
        ...
        if store is not None:
            candidates = [
                store.load_session(str(row.get("session_id") or ""))
                for row in store.list_conversations(limit=100)
            ]
        else:
            candidates = host.list_source_sessions(limit=100)
        for candidate in candidates:
            if not isinstance(candidate, dict):
                continue
            if source_runtime_type(candidate):
                if self._branch_runner is not spawn_native_agent_branch:
                    return candidate
                try:
                    resolve_factory(candidate)
                    return candidate
                except Exception:
                    continue
