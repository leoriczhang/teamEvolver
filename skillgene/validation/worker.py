...
class ValidationWorker:
    ...
    async def _score_replay_output_with_rubric(
        ...
        try:
            raw = await self._client.chat(messages, max_tokens=512, temperature=0.0)
            ...
            return {
                "score": round(score, 3),
                "signal": "gold_rubric",
                "reason": str(parsed.get("reason") or "internal benchmark rubric score")[:800],
            }
        except Exception:  # noqa: BLE001 - deterministic generic fallback remains available
            return self._score_replay_output(response_text)
    ...
    async def _run_replay_branch(
        ...
        *,
        ...
        replay_result = await self._score_replay_output_with_rubric(case, response_text)
