...
SkillMiner (``skillgene/skillminer``) ships its own zero-dependency console
...
SkillGene FastAPI app under a ``/api/mining`` prefix so the unified console can
...
class SkillMinerBridgeMixin:
    ...
    ``self.config``. Enabled unless ``SKILLGENE_SKILLMINER_ENABLED`` is falsy.
    """
    ...
    def _skillminer_enabled(self) -> bool:
        raw = os.environ.get("SKILLGENE_SKILLMINER_ENABLED", "1").strip().lower()
    ...
    def _start_skillminer(self) -> None:
        ...
        with self._skillminer_lock:
            if self._skillminer_proc is not None and self._skillminer_proc.poll() is None:
                return
            port = _pick_free_port(int(os.environ.get("SKILLGENE_SKILLMINER_PORT", "8765") or 8765))
            env = dict(os.environ)
            env["PORT"] = str(port)
            # Ensure the mining API key falls through from SkillGene config if set.
    ...
    async def _dispatch_skillminer_request(self, request: Request) -> Optional[Response]:
        ...
        if isinstance(console_user, dict):
            ...
            headers["X-SkillGene-Reviewer"] = reviewer[:120]
