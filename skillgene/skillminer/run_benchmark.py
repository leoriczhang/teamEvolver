...
def parse_difficulty_dist(spec):
    ...
    try:
        ...
        return dict(DEFAULT_DIFFICULTY_DIST)
    total = sum(max(0.0, raw.get(l, 0.0)) for l in DIFFICULTY_LEVELS)
    if total <= 0:
        return dict(DEFAULT_DIFFICULTY_DIST)
    return {l: max(0.0, raw.get(l, 0.0)) / total for l in DIFFICULTY_LEVELS}


def dist_to_counts(dist, total):
    """把归一化比例 + 目标题量换算成各档目标题数（最大余数法保证求和精确）。"""
    exact = {l: dist[l] * total for l in DIFFICULTY_LEVELS}
    floor = {l: int(exact[l]) for l in DIFFICULTY_LEVELS}
    remainder = total - sum(floor.values())
    # 余数按小数部分从大到小分配
    order = sorted(DIFFICULTY_LEVELS, key=lambda l: exact[l] - floor[l], reverse=True)
...
def build_phase(skill_dir, skill_name, hermes_env, difficulty_counts=None):
    ...
    # benchmark.jsonl 是 SkillMiner → SkillGene 候选评审的内部数据契约。
...
def judge_prompt_dialogue(question, transcript, meta, eval_text):
    ...
    return (
        ...
        '  "info_gathering": {"should_ask": ["本应问出的关键信息"], "actually_asked": ["实际问到的"], "missed": ["漏问的"]},\n'
        '  "must_hit_covered": ["已命中的要点"],\n'
        '  "must_hit_missed": ["未命中的要点"],\n'
        '  "must_avoid_violated": ["被触犯的禁项，没有则空数组"],\n'
        '  "safety_gate_passed": true,\n'
...
def aggregate(results, difficulty_target=None):
    ...
    diff_actual = {l: 0 for l in DIFFICULTY_LEVELS}
    diff_actual["other"] = 0
    diff_score = {l: [0.0, 0] for l in DIFFICULTY_LEVELS}  # level -> [score_sum, scored_n]
    ...
    for l in DIFFICULTY_LEVELS:
        s, cnt = diff_score[l]
        diff_score_rate[l] = round(s / cnt * 100, 1) if cnt else None

    # 难度分布：目标 vs 实际（占比）
    diff_target = None
    if difficulty_target:
        tt = sum(difficulty_target.values())
        diff_target = {l: {"count": difficulty_target.get(l, 0),
                           "pct": round(difficulty_target.get(l, 0) / tt * 100, 1) if tt else 0.0}
                       for l in DIFFICULTY_LEVELS}
    diff_actual_pct = {l: (round(diff_actual[l] / n * 100, 1) if n else 0.0)
                       for l in DIFFICULTY_LEVELS}
...
def write_report(agg, results, skill_name):
    ...
    if dt:
        ...
        for l in DIFFICULTY_LEVELS:
            sr = dsr[l]
            lines.append(
                f"| {l} | {dt[l]['count']} | {dt[l]['pct']}% | {da[l]} | {dap[l]}% "
