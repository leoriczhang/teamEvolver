...
vendored into SkillGene: it currently has no license file and brings a separate
...
LIFT_DATASETS_ROOT = Path(
    os.environ.get("SKILLGENE_LIFT_DATASETS_DIR", str(PROJECT_ROOT / "lift_datasets"))
).expanduser().resolve()
...
def _slug(value: str, fallback: str = "skillgene-suite") -> str:
...
def lift_status() -> dict[str, Any]:
    ...
    return {
        ...
        "schema": LIFT_SCHEMA,
        "root": str(root),
        "root_configured": bool(os.environ.get("SKILLGENE_LIFT_ROOT", "").strip()),
...
def build_suite_from_questions(
    ...
    *,
    ...
    extra_skills_dir = f"assets/benchmark_mds/skillgene/{suite_slug}/skills"
    suite = {
...
def _align_extra_skills_dir(suite: dict[str, Any]) -> None:
    extra_skills_dir = f"assets/benchmark_mds/skillgene/{_slug(suite['name']).lower()}/skills"
...
def _backup_existing(root: Path, scene_target: Path, json_target: Path) -> Path | None:
    if not scene_target.exists() and not json_target.exists():
        return None
    backup_root = root / "assets" / "skillgene_import_history" / _timestamp()
...
def publish_draft(draft_id: str) -> dict[str, Any]:
    ...
    if not status["checkout_ready"]:
        raise FileNotFoundError(
            "LIFT checkout 未就绪。请设置 SKILLGENE_LIFT_ROOT，或运行 scripts/setup_lift.sh"
        )
    root = Path(status["root"])
    suite_slug = _slug(suite["name"]).lower()
    scene_target = root / "assets" / "benchmark_mds" / "skillgene" / suite_slug
    json_target = root / "assets" / "benchmarks" / "skillgene" / f"{suite_slug}.json"
...
def build_lift_command(options: dict[str, Any]) -> tuple[list[str], dict[str, Any]]:
    ...
    run_suffix = _slug(str(options.get("run_id") or f"skillgene-{_timestamp()}"))
    root = Path(status["root"])
    suite_path = root / "assets" / "benchmarks" / "skillgene" / suite_file
