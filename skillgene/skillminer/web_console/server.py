...
class RunManager:
    ...
    def _run_real(self, cfg):
        ...
        for round_idx in range(1, max_rounds + 1):
            ...
            if self.stop_flag.is_set():
                stop_reason = "使用者手动中止"; break

            if info["is_production"]:
                stop_reason = "已达生产级，反思环收敛"; break
            if skill_md is None or info["confidence"] == "unknown":
                stop_reason = "未解析到有效 SKILL.md/置信档，停止"; break
            if round_idx >= max_rounds:
                stop_reason = f"达到最大轮数 {max_rounds}，停止"; break
            if prev_gap is not None and info["gap_count"] >= len(prev_gap[1]):
                stop_reason = f"缺口数未下降，判定收敛，停止"; break
    ...
    def _run_benchmark(self, opts):
        ...
        try:
            ...
            if self.stop_flag.is_set():
                self.emit("bench_done", stopped=True); self.state = "done"; return
        ...
        if path == "/api/sources/upload":
            ...
            try:
                if action == "save":
                    ...
                    reviewer = str(
                        self.headers.get("X-SkillGene-Reviewer")
